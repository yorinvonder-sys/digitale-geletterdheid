...
| **AI-model** | Mistral AI voor tekst/chat/feedback/vision/OCR; Black Forest Labs FLUX voor beeldgeneratie; providerregio en DPA per provider te verifiëren |
...
| R03 | **Schadelijke content voor minderjarigen** — AI genereert gewelddadige, seksueel expliciete, of anderszins schadelijke content voor 12-18 jarigen | Veiligheid | 2 | 5 | **10 HOOG** | DGSkills outputfiltering, missiespecifieke instructies en providermaatregelen waar beschikbaar, system instructions beperken agentrol, prompt sanitizer blokkeert manipulatiepogingen | **LAAG (4)** — Providermaatregelen en DGSkills-outputfiltering beperken bekende schadelijke output, maar zijn niet foutloos. Restrisco zit in edge cases waar content technisch niet wordt geblokkeerd maar contextueel ongepast is voor minderjarigen. | Actief | Monitor Mistral AI en Black Forest Labs model-/policy-updates op wijzigingen in filtergedrag, voer periodiek red-teaming uit met leeftijdsspecifieke scenario's |
...
| R10 | **Chatinhoud lekt naar derden** — Leerlingberichten worden opgeslagen of gebruikt voor provider-modeltraining door externe AI-providers | Privacy | 1 | 5 | **5 MIDDEN** | Server-side AI-providerproxy, dataminimalisatie, outputfiltering; DPA/regio/retentie/subprocessorroute en modeltraining-uitsluiting per provider nog aantoonbaar vastleggen | **LAAG (2)** — Restrisico blijft totdat providerbewijs per Mistral AI en Black Forest Labs volledig is vastgelegd en periodiek wordt herbeoordeeld. | Beheerst | Monitor provider-DPA en subprocessorwijzigingen, documenteer datastromen in technische documentatie |
...
| R21 | **Federated-identity vertrouwen** — Een opt-in-school dwingt in werkelijkheid geen MFA af in haar Entra-tenant, waardoor een SSO-login zonder tweede factor toegang krijgt tot docent- of beheerdersdata | Privacy/Security | 2 | 4 | **8 MIDDEN** | Per-school opt-in vereist schriftelijke attestatie; DGSkills' eigen TOTP blijft de standaard voor e-mail/wachtwoord-logins en voor scholen zonder attestatie | **MIDDEN** — Geen per-login amr/acr-verificatie in v1; hardening (amr/acr-claimcontrole) is voorzien voor opschaling | Actief | Verifieer attestatie per school bij onboarding; implementeer amr/acr-claimcontrole vóór opschaling naar meer dan vijf opt-in-scholen |
...
**Specifieke mitigatie:** Welzijnsprotocol actief; DGSkills-outputfiltering en providermaatregelen waar beschikbaar. System instruction moet worden aangescherpt met expliciete grenzen over welke onderwerpen besproken mogen worden en op welk detailniveau.
...
**Specifieke mitigatie:** System instruction beperkt uitleg tot herkenning, niet creatie. DGSkills-outputfiltering en providermaatregelen beperken gedetailleerde "how-to" instructies.
...
| **TM-03: AI-outputfiltering en providermaatregelen** | Server-side outputfiltering, missiespecifieke instructies en providermaatregelen waar beschikbaar. | R03, R04 | Geimplementeerd / providerafhankelijk |
| **TM-04: Rate Limiting** | Durable rate limiter: 15 requests per minuut per gebruiker (server-side, gekoppeld aan user ID). AI-provider 429-responses worden afgehandeld. | R13 | Geimplementeerd |
...
| **Jaarlijks** | Review van AI-provider contractuele voorwaarden, DPA, subprocessors en retentie | Aanbieder (DGSkills) |
...
| **Mistral AI / Black Forest Labs model- of policy-update** | Regressietest op alle agents, beoordeel impact op beoordelingskwaliteit en veiligheidsfilters | Binnen 1 week na update |
