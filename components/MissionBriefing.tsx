...
export const MissionBriefing: React.FC<MissionBriefingProps> = ({ role, onStart, onBack }) => {
    ...
    return (
        <div className="w-full h-full max-w-3xl mx-auto flex items-center justify-center px-4 sm:px-6 py-2 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="w-full bg-white rounded-2xl shadow-xl border border-lab-muted overflow-hidden flex flex-col h-auto max-h-[90vh]">
                ...
                <div className="relative flex-1 p-6 md:p-10 flex flex-col justify-between overflow-y-auto custom-scrollbar">
                    ...
                    {/* Header */}
                    <div className="flex items-center gap-4 mb-6 border-b border-lab-muted pb-6 shrink-0">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs font-bold bg-lab-bg text-lab-textLight px-2 py-0.5 rounded-full uppercase tracking-wider border border-lab-muted inline-flex">{role.difficulty} Level</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="mb-8 space-y-4 animate-in slide-in-from-top-4 fade-in duration-500">
                            <div className="bg-lab-bg p-4 rounded-xl border-l-4 border-lab-primary">
                                <div className="flex flex-col md:flex-row gap-6">
                                    {role.briefingImage && (
                                        <div className="w-full md:w-1/3 h-40 rounded-xl overflow-hidden shadow-sm shrink-0 bg-lab-muted">
                                            {imageError ? (
                            ...
                            <div className="flex items-center gap-3 text-lab-primary font-bold text-base bg-lab-bg p-3 rounded-xl border border-lab-muted">
                        ...
                        <div key={currentStepIndex} className="animate-in slide-in-from-right-8 fade-in duration-300">
                            {currentStep && (
                                <>
                                    ...
                                    <div className="bg-white border-2 border-lab-muted p-4 rounded-2xl flex items-start gap-4 shadow-sm">
                                        <div className="mt-1">
                    ...
                    {/* Navigation Controls */}
                    <div className="mt-8 flex items-center gap-4 pt-6 border-t border-lab-muted shrink-0">
                        <button
                            onClick={handlePrev}
                        ...
                        <div className="flex gap-2 flex-1 justify-center">
                            {steps.map((_, i) => (
                                <div key={i} className={`h-2 rounded-full transition-all duration-300 ${i === currentStepIndex ? 'w-8 bg-lab-primary' : 'w-2 bg-lab-muted'}`} />
                            ))}
                        </div>

                        {isLastStep && readCountdown > 0 ? (
                            <div className="px-8 py-3 bg-lab-muted text-lab-textLight rounded-full font-bold text-sm flex items-center gap-3 cursor-not-allowed">
