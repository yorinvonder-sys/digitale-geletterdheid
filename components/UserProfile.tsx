            ...
            <div className="w-full max-w-[95%] lg:max-w-[1920px] mx-auto p-4 lg:p-8">
                ...
                {onboardingStep !== null ? (
                    <div className="flex-1 bg-white rounded-[3rem] shadow-2xl border border-lab-muted overflow-hidden flex flex-col md:flex-row h-[85dvh] min-h-[600px] animate-in zoom-in duration-500 relative">
                        ...
                        <div className="md:w-1/2 bg-lab-muted relative border-r border-lab-muted">
                            <LazyAvatarViewer
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-[calc(100dvh-180px)] min-h-[600px]">
                        <div className="lg:col-span-5 flex flex-col gap-6 h-full">
                            <div className="flex-1 bg-gradient-to-b from-slate-200 to-white rounded-[2.5rem] p-1 shadow-xl relative overflow-hidden border border-white">
                                <div className="w-full h-full relative rounded-[2.3rem] overflow-hidden bg-lab-muted">
                            ...
                            <div className="bg-white p-2 rounded-2xl shadow-sm border border-lab-muted flex gap-2">
                                <button onClick={() => setActiveTab('profile')} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'profile' ? 'bg-indigo-600 text-white shadow-lg' : 'text-lab-muted hover:bg-lab-muted'}`}><User size={16} /> Stats</button>
                                <button onClick={() => setActiveTab('shop')} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'shop' ? 'bg-indigo-600 text-white shadow-lg' : 'text-lab-muted hover:bg-lab-muted'}`}><ShoppingBag size={16} /> Winkel</button>
                                <button onClick={() => setActiveTab('trophies')} className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'trophies' ? 'bg-indigo-600 text-white shadow-lg' : 'text-lab-muted hover:bg-lab-muted'}`}><Trophy size={16} /> Trofeeën</button>
                            </div>
                        </div>

                        <div className="lg:col-span-7 bg-white rounded-[2.5rem] p-8 shadow-sm border border-lab-muted overflow-y-auto custom-scrollbar">
                            {activeTab === 'trophies' ? (
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-12">
                                        ...
                                        {(shopCategory === 'all' || shopCategory === 'colors') && (
                                            <div className="space-y-6 animate-in fade-in duration-500">
                                                ...
                                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-lab-muted/50 p-6 rounded-[2.5rem] border border-lab-muted">
                                                    <CircularColorPicker
                                                        size="sm"
            ...
            {itemToPurchase && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="relative bg-lab-muted h-64 border-b border-lab-muted">
