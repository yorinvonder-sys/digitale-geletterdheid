...
const LABEL_CONFIG: Record<DeveloperNoteLabel, {
    ...
    werklog: {
        ...
        btnInactive: 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100',
    },
    handoff: {
        label: 'Handoff',
        ...
        btnActive: 'bg-orange-500 text-white',
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            {lastHandoff && (
                <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5 flex gap-4 items-start">
                    <div className="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center shrink-0">
                        <ArrowRightCircle size={20} className="text-orange-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest mb-1">Laatste handoff — hoe verder</p>
                        <p className="text-sm text-orange-900 leading-relaxed whitespace-pre-wrap line-clamp-3">
                            {lastHandoff.body}
                        </p>
                        <p className="text-[10px] text-orange-400 mt-1.5" title={fullDate(lastHandoff.createdAt)}>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Nieuwe notitie</h3>
                ...
                <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 outline-none resize-none transition-all"
                />
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                        ...
                        className="ml-auto inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                    >
                        <Save size={14} />
                    ...
                    return (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all capitalize ${
                                isActive
                                    ? (cfg ? cfg.btnActive : 'bg-slate-800 text-white')
                                    : (cfg ? cfg.btnInactive : 'bg-slate-100 text-slate-600 hover:bg-slate-200')
                            }`}
                        >
            ...
            {filtered.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
                    <div className="space-y-3">
                        <p className="text-slate-500 font-bold">Nog geen notities</p>
                        <p className="text-sm text-slate-400 max-w-sm mx-auto leading-relaxed">
                            Gebruik <span className="font-bold text-blue-600">Gedachte</span> voor ideeën en twijfels,{' '}
                            <span className="font-bold text-emerald-600">Werklog</span> voor wat je gedaan hebt, en{' '}
                            <span className="font-bold text-orange-500">Handoff</span> om morgen meteen te weten hoe je verder gaat.
                        </p>
                    </div>
                </div>
            ) : (
                                        ...
                                        {isEditing ? (
                                            <div className="space-y-3">
                                                ...
                                                <textarea
                                                    ...
                                                    rows={4}
                                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none resize-none transition-all"
                                                    autoFocus
                                                />
                                                <div className="flex gap-2">
                                                    <button
                                                        ...
                                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                                                    >
                                                        <Save size={12} />
                                                    ...
                                                    <button
                                                        onClick={cancelEdit}
                                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-200 transition-all"
                                                    >
                                                        <X size={12} />
                                                        Annuleren
                                                    </button>
                                            ...
                                            <p
                                                className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap cursor-pointer hover:text-slate-600 transition-colors"
