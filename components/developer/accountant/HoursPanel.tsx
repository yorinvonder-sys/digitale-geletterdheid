...
export function HoursPanel({ userId, year }: HoursPanelProps) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-lab-muted text-white rounded-[2rem] p-4 sm:p-8">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    {criteriumHaald && (
                        <div className="ml-auto flex items-center gap-2 bg-lab-sage/20 text-lab-sage rounded-xl px-4 py-2">
                            <CheckCircle size={16} />
                            <span className="text-xs font-black uppercase tracking-wide">Gehaald</span>
                        </div>
                    )}
                    {!criteriumHaald && totalHours > 0 && (
                        <div className="ml-auto flex items-center gap-2 bg-lab-gold/20 text-lab-gold rounded-xl px-4 py-2">
                            <AlertCircle size={16} />
                ...
                <div className="mb-3">
                    <div className="h-4 bg-white/10 rounded-full overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all duration-500 ${criteriumHaald ? 'bg-lab-sage' : 'bg-lab-gold'}`}
            ...
            <div className="bg-white rounded-[2rem] border border-lab-muted shadow-sm p-4 sm:p-8">
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 bg-indigo-100 rounded-xl flex items-center justify-center">
                        <Plus size={20} className="text-indigo-600" />
                    </div>
                    <h3 className="text-lg font-black text-lab-muted uppercase tracking-tight">Uren registreren</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2.5 border border-lab-muted rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
                        />
                    </div>
                ...
                <div className="flex items-center justify-between mt-5">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                        <div
                            ...
                            className={`relative w-11 h-6 rounded-full transition-colors ${form.billable ? 'bg-lab-sage' : 'bg-lab-muted'}`}
                        >
                            <span
                    ...
                    {byMonth.map(({ month, hours }) => {
                        ...
                        return (
                            <div key={month} className="flex-1 flex flex-col items-center gap-1">
                                {/* Maandafkorting */}
                                <span className={`text-[9px] font-black uppercase tracking-wide ${isCurrentMonth ? 'text-indigo-600' : 'text-lab-muted'}`}>
                                    {MAAND_AFKORTINGEN[month - 1]}
                                </span>
                                {/* Bar container */}
                                <div className="w-full flex items-end justify-center" style={{ height: '64px' }}>
                                    <div
                                        className={`w-full rounded-t-lg transition-all duration-300 ${hours > 0 ? (isCurrentMonth ? 'bg-indigo-500' : 'bg-indigo-200') : 'bg-lab-muted'}`}
            ...
            <div className="bg-white rounded-[2rem] border border-lab-muted shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-lab-muted flex items-center justify-between">
                ...
                {entries.length === 0 ? (
                    <div className="py-16 text-center">
                        <div className="w-14 h-14 bg-lab-muted rounded-2xl flex items-center justify-center mx-auto mb-4">
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {entries.map(entry => (
                                    <tr
                                        key={entry.id}
                                        className="hover:bg-lab-muted transition-colors group"
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className="text-sm font-black text-lab-muted tabular-nums">
                                                {entry.hours.toLocaleString('nl-NL', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            {entry.billable ? (
                                                <CheckCircle size={18} className="text-lab-sage mx-auto" />
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button
                                                ...
                                                className={`p-2.5 rounded-lg text-lab-muted hover:text-red-500 hover:bg-red-50 transition-all ${hoveredRow === entry.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                                                title="Verwijderen"
                                            >
                                                <Trash2 size={16} />
                                            </button>
                            ...
                            {entries.length > 0 && (
                                <tfoot>
                                    <tr className="border-t-2 border-lab-muted bg-lab-muted">
...
//                <div
//                    className={`h-full rounded-full transition-all ${criteriumHaald ? 'bg-lab-sage' : 'bg-lab-gold'}`}
...
//                <CheckCircle size={18} className="text-lab-sage shrink-0 mt-0.5" />
//                <div>
