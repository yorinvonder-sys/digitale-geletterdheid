      ...
      {onWeightChange && (
        <div className="mt-3 space-y-2 px-2">
          ...
          {weights.slice(0, activeCount).map((w, i) => (
            <div key={`slider-${i}`} className="flex items-center gap-2 bg-[#FCF6EA] rounded-lg px-3 py-2 border border-[#FCF6EA]">
              ...
              />
              <span className={`text-xs font-bold w-10 text-right ${w >= 0 ? 'text-[#5F947D]' : 'text-red-500'}`}>
                {w.toFixed(1)}
              </span>
            </div>
          ))}
      ...
      <div className="min-h-screen bg-[#FCF6EA] overflow-y-auto pb-safe" style={{ fontFamily: "'Outfit', system-ui, sans-serif" }}>
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#E7D8BD]">
            ...
            <div className="mt-3 bg-[#FCF6EA] rounded-xl p-3 border border-[#FCF6EA]">
          ...
          <div className={`rounded-2xl p-4 border-2 text-center transition-all duration-300 ${isClose ? 'bg-[#5F947D]/10 border-[#5F947D]' : 'bg-white border-[#E7D8BD]'}`}>
            ...
            <div className="flex items-center justify-center gap-4">
              <div className="flex-1 max-w-[200px]">
                <div className="w-full bg-[#FCF6EA] rounded-full h-3 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-200 ${output > 0.7 ? 'bg-[#5F947D]' : output < 0.3 ? 'bg-red-500' : 'bg-[#D97848]'}`}
          ...
          {showFeedback && (
            <div className={`rounded-2xl p-4 border-2 ${lastFeedbackCorrect ? 'bg-[#5F947D]/10 border-[#5F947D]/30' : 'bg-red-500/5 border-red-500/20'}`}>
              <div className="flex items-start gap-3">
                {lastFeedbackCorrect ? (
                ...
                <div>
                  <h3 className={`text-sm font-bold ${lastFeedbackCorrect ? 'text-[#5F947D]' : 'text-red-500'}`}>
              ...
              <div className="flex gap-1 mt-2">
                {challengeResults.map((r, i) => (
                  <div key={i} className="flex items-center gap-1">
                    <span className={`text-[10px] font-bold ${r ? 'text-[#5F947D]' : 'text-red-500'}`}>
                      {CHALLENGES[i]?.difficulty === 'easy' ? 'Basis' : CHALLENGES[i]?.difficulty === 'medium' ? 'Gem.' : 'Expert'}
                    </span>
                    {r ? <CheckCircle size={12} className="text-[#5F947D]" /> : <XCircle size={12} className="text-red-500" />}
                  </div>
                ))}
              </div>
            </div>
