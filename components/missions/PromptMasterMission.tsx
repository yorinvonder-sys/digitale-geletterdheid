            ...
            <div className={`p-4 rounded-2xl border ${getBgColor()} h-full flex flex-col`}>
                ...
                <div className="flex-1 min-h-[200px] bg-[#FCF6EA] rounded-xl flex flex-col items-center justify-center p-4 text-center relative overflow-hidden group">
                    ...
                    {imageToShow ? (
                        ...
                        <>
                            <div className="absolute inset-0 bg-gradient-to-br from-[#FCF6EA] to-[#E7D8BD] opacity-50" />
                            {/* Abstract shapes to simulate image */}
                            <div className="absolute top-1/4 left-1/4 w-16 h-16 bg-[#D97848]/10 rounded-full blur-xl" />
                            <div className="absolute bottom-1/3 right-1/4 w-24 h-24 bg-[#0B453F]/10 rounded-full blur-2xl" />

                            <Image size={32} className={`mb-3 relative z-10`} style={{ color: isIdeal || isSuccess ? '#5F947D' : '#D97848' }} />
                            <p className={`text-sm relative z-10 font-medium`} style={{ fontFamily: "'Outfit', system-ui, sans-serif", color: isIdeal || isSuccess ? '#5F947D' : '#D97848' }}>
                                {content.replace(/^De AI.*?:\s*/i, '')}
                            </p>
