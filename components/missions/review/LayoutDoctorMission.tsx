            ...
            case 'Ontwerpen':
                return (
                    <div className="px-4 py-2 flex items-center gap-4 h-24">
                        ...
                        <button onClick={() => { setBodyFont('sans'); setPageColor('#fdf6e3'); }} className="flex flex-col items-center hover:bg-[#FCF6EA] p-2 rounded-lg transition-all duration-300">
        ...
        <div className="min-h-screen bg-[#FCF6EA] flex flex-col text-[#08283B]" style={{ fontFamily: "'Outfit', system-ui, sans-serif" }} onClick={() => setSelection('none')}>
            ...
            <header className="bg-[#D97848] text-white px-4 py-2 flex items-center justify-between shadow-md relative z-20">
                ...
                <button
                    ...
                    className={`px-4 py-1.5 rounded-full font-bold text-sm flex items-center gap-2 transition-all duration-300 focus-visible:ring-2 focus-visible:ring-white ${isComplete ? 'bg-[#5F947D] hover:bg-lab-sage' : 'bg-white/20'}`}
                >
            ...
            {/* Ribbon */}
            <div className="bg-white border-b border-[#E7D8BD] shadow-sm relative z-10" onClick={(e) => e.stopPropagation()}>
                {renderRibbon()}
            </div>

            {/* Workspace */}
            <div className="flex-1 overflow-y-auto p-8 flex justify-center bg-[#FCF6EA] perspective-[1000px]">
                <div
                ...
                >
                    ...
                    <motion.div
                        ...
                        className={`absolute w-48 h-32 bg-[#E7D8BD] z-20 cursor-move ${selection === 'image' ? 'ring-2 ring-[#D97848]' : ''}`}
                            ...
                            <div key={block.id} className="mb-4 break-inside-avoid-column">
                                {block.type === 'heading' && (
                                    <h1
                                        ...
                                        className={`cursor-text ${titleStyle === 'comic' ? 'text-2xl text-green-600 text-center font-[Comic_Sans_MS]' : 'text-4xl font-bold border-b-2 border-[#08283B] pb-2 text-left font-sans'} ${selection === block.id ? 'bg-[#D97848]/10' : ''}`}
                                    >
                                        {block.content}
                                    </h1>
                                )}
