    ...
    return (
        <div>
            <div className="max-w-3xl mx-auto">
                <h2 className="text-2xl md:text-3xl font-medium tracking-tight mb-3" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#08283B' }}>
                    Veelgestelde vragen
                </h2>
                <p className="text-base leading-relaxed mb-10" style={{ color: '#445865' }}>
                ...
                <div className="space-y-8">
                    {(['Over het platform', 'Technisch & privacy', 'Kosten & licentie'] as const).map(cat => {
                        ...
                        return (
                            <div key={cat}>
                                <h3 className="text-sm font-medium uppercase tracking-wide mb-4" style={{ color: '#D97848' }}>{cat}</h3>
                                <div className="space-y-3">
                                    {items.map((item) => {
                                        ...
                                        return (
                                            <div key={i} className="bg-white rounded-xl overflow-hidden shadow-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#E7D8BD' }}>
                                                <button
                                                    onClick={() => setOpenFaq(isOpen ? null : i)}
                                                    className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-[#FCF6EA]/50 transition-colors focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#D97848] focus-visible:rounded-xl"
                                                    aria-expanded={isOpen}
                                                    aria-controls={`faq-answer-${i}`}
                                                >
                                                    <span className="text-[15px] font-medium pr-4 leading-snug" style={{ color: '#08283B' }}>{item.q}</span>
                                                    <IconChevronDown className={`flex-shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} style={{ color: '#445865' }} />
                                                </button>
                                                {isOpen && (
                                                    <div id={`faq-answer-${i}`} className="px-5 pb-5 pt-0">
                                                        <p className="text-[15px] leading-relaxed" style={{ color: '#445865' }}>{item.a}</p>
                                                        {showCta && (
                                                            <button
                ...
                <div className="mt-12 pt-8 text-center" style={{ borderTopWidth: '1px', borderTopStyle: 'solid', borderTopColor: '#E7D8BD' }}>
                    <p className="text-sm mb-4" style={{ color: '#445865' }}>Nog vragen? We staan voor je klaar.</p>
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <button
                            onClick={scrollToContact}
                            className="text-sm font-semibold text-white px-5 py-2.5 rounded-full inline-flex items-center gap-1.5 transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#D97848]"
                        ...
                        <a
                            ...
                            style={{ color: '#445865', borderWidth: '1px', borderStyle: 'solid', borderColor: '#E7D8BD' }}
