...
export const ActivityFeed: React.FC<ActivityFeedProps> = ({ schoolId }) => {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            case 'mission_complete': return <Target size={14} className="text-lab-sage" />;
            case 'xp_earned': return <Zap size={14} className="text-lab-gold" />;
    ...
    return (
        <div className="bg-white rounded-3xl border border-lab-muted shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-4 border-b border-lab-muted flex items-center justify-between">
                <h3 className="text-xs font-black text-lab-muted uppercase tracking-widest flex items-center gap-2">
                    <div className="w-2 h-2 bg-lab-sage rounded-full animate-pulse"></div>
                    Live Activiteit
                </h3>
                <span className="text-[10px] font-bold text-lab-muted uppercase tracking-tighter">Real-time</span>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {activities.length === 0 ? (
                    <div className="text-center py-12 text-lab-muted text-xs italic">Nog geen recente activiteit...</div>
                ) : activities.map((activity) => (
                    <div key={activity.id} className="flex gap-3 group">
                        <div className="mt-1 w-8 h-8 rounded-xl bg-lab-muted border border-lab-muted flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            {getIcon(activity.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                                <span className="text-sm font-bold text-lab-muted truncate">{activity.studentName}</span>
                                <span className="text-[9px] font-medium text-lab-muted whitespace-nowrap flex items-center gap-1">
                                    <Clock size={8} />
                                    {activity.timestamp ? new Date(activity.timestamp).toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit' }) : 'nu'}
                                </span>
                            </div>
                            <p className="text-xs text-lab-muted font-medium">{activity.data}</p>
                        </div>
                    </div>
                ))}
            </div>
