    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4 items-center justify-between">
                <div className="flex items-center gap-4">
                    {/* Class Filter */}
                    <div className="flex items-center gap-2 bg-white rounded-xl border border-lab-muted px-3 py-2 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {/* Feedback Count */}
                    <div className="bg-white rounded-xl border border-lab-muted px-4 py-2 shadow-sm">
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-dashed border-lab-muted p-12 text-center">
                ...
                <div className="space-y-3">
                    {filteredFeedback.map((item) => (
                        <div
                            key={item.id}
                            className={`bg-white rounded-2xl border p-5 shadow-sm hover:shadow-md transition-shadow relative ${selectedIds.has(item.id!) ? 'border-indigo-500 ring-1 ring-indigo-500 bg-indigo-50' : 'border-lab-muted'}`}
                        >
                            <div className="flex items-start gap-4">
                                <button
                                    onClick={() => item.id && toggleSelect(item.id)}
                                    className="mt-1 text-lab-muted hover:text-indigo-600 transition-colors shrink-0"
                                >
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            <div className="font-bold text-lab-muted">{item.user_name}</div>
                                            <div className="text-xs text-lab-muted flex items-center gap-2">
                                                <span className="bg-lab-muted px-2 py-0.5 rounded-full">{item.user_class}</span>
                                    ...
                                    <p className="text-lab-muted text-sm leading-relaxed bg-lab-muted p-4 rounded-xl">
                                        {item.message}
                                    </p>
                                </div>
                                ...
                                <button
                                    ...
                                    className="p-2 text-lab-muted hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors shrink-0"
                                    title="Verwijderen"
                                >
                                    {deleting === item.id ? (
