            ...
            {showMessageModal && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowMessageModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-black text-lab-muted mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-4">
                                    ...
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition-all ${messageTarget === opt.id
                                                ...
                                                : 'border-lab-muted text-lab-muted hover:border-lab-muted'
                                                }`}
                                        >
                                            <opt.icon size={18} />
                            ...
                            {(messageTarget === 'class' || messageTarget === 'student') && (
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full p-3 border border-lab-muted rounded-xl text-sm font-medium bg-white"
                                    >
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full p-3 border border-lab-muted rounded-xl text-sm resize-none h-28 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none"
                                ></textarea>
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    ...
                                    className="flex-1 py-3 text-lab-muted font-bold rounded-xl hover:bg-lab-muted transition-colors"
                                >
                                    Annuleer
                                </button>
                                <button
            ...
            {showBadgeModal && selectedStudent && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowBadgeModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-black text-lab-muted mb-4">Badge Toekennen</h3>
                        <div className="grid grid-cols-4 gap-2 mb-4">
                            {AVAILABLE_BADGES.map(badge => (
                                <button
                                    key={badge.id}
                                    onClick={() => setSelectedBadge(badge.id)}
                                    className={`p-3 rounded-xl text-2xl transition-all ${selectedBadge === badge.id ? 'bg-lab-gold ring-2 ring-amber-400' : 'bg-lab-muted hover:bg-lab-muted'}`}
                                    title={badge.name}
                                >
                                    {badge.emoji}
                                </button>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowBadgeModal(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-muted">Annuleer</button>
                            <button onClick={handleAwardBadge} disabled={!selectedBadge} className="flex-1 py-2 bg-lab-gold text-white font-bold rounded-xl hover:bg-lab-gold disabled:opacity-50">Toekennen</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowResetConfirm(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        <div className="text-center mb-4">
                            ...
                            <h3 className="text-lg font-black text-lab-muted">Weet je het zeker?</h3>
                            <p className="text-sm text-lab-muted mt-2">Dit reset alle XP, level en voltooide missies van {selectedStudent.displayName}.</p>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-muted">Annuleer</button>
            ...
            {showHighlightModal && selectedStudent && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHighlightModal(false)}>
                    <div className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-8" onClick={e => e.stopPropagation()}>
                        <div className="w-16 h-16 bg-lab-gold text-lab-gold rounded-2xl flex items-center justify-center mb-6">
                            <Trophy size={32} />
                        </div>
                        <h3 className="text-2xl font-black text-lab-muted mb-2">Kroon op het Werk</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-muted px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Titel van de prestatie</label>
                                <input
                                    type="text"
                                    value={highlightTitle}
                                    onChange={(e) => setHighlightTitle(e.target.value)}
                                    placeholder="Bijv. Meesterlijke Prompt..."
                                    className="w-full p-4 border-2 border-lab-muted rounded-2xl text-sm font-bold focus:border-lab-gold outline-none transition-all"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-muted px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Inhoud / Prompt</label>
                                <textarea
                                    value={highlightContent}
                                    onChange={(e) => setHighlightContent(e.target.value)}
                                    placeholder="Wat was er zo goed?"
                                    className="w-full p-4 border-2 border-lab-muted rounded-2xl text-sm font-bold focus:border-lab-gold outline-none transition-all resize-none h-32"
                                ></textarea>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-muted px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Jouw compliment (Optioneel)</label>
                                <input
                                    type="text"
                                    value={highlightNote}
                        ...
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowHighlightModal(false)} className="flex-1 py-4 text-lab-muted font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-lab-muted transition-all">Annuleer</button>
                            <button
                                onClick={handleHighlightWork}
                                disabled={!highlightTitle || !highlightContent}
                                className="flex-1 py-4 bg-lab-gold text-white font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-lab-gold shadow-lg shadow-amber-200 disabled:opacity-50 transition-all"
                            >
                                In de Gallerij
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full p-2 border border-lab-muted rounded-xl text-sm"
                                />
                            </div>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setShowEventModal(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-muted">Annuleer</button>
            ...
            {studentToDelete && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setStudentToDelete(null)}>
                    <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <h3 className="text-lg font-black text-center text-lab-muted mb-2">
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-2.5 bg-lab-muted text-lab-muted font-bold rounded-xl hover:bg-lab-muted transition-colors"
                                disabled={isDeleting}
                            >
                                Annuleren
                            </button>
