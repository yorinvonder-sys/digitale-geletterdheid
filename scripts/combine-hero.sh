#!/bin/bash
# Combine real screenshots slideshow + beaver green screen overlay
...
echo "   Screenshots + beaver green screen overlay"
echo ""
...
# Then overlay beaver green screen with chromakey
...
$FFMPEG -y \
  ...
  -i "$VIDEOS/beaver-greenscreen.mp4" \
  -filter_complex "
    ...
    [4:v]chromakey=0x00FF00:0.25:0.1,scale=1280:720[beaver];
    [slideshow][beaver]overlay=0:0:shortest=1[out]
  " \
  -map "[out]" \
