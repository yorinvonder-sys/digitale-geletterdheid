...
const SITE_ORIGIN = 'https://dgskills.app';
...
const SITEMAP_ROUTES = ROUTES.filter((route) => route !== '/404');
...
function prerenderRoutes() {
  ...
  const routeMeta = {
    ...
    '/ict': {
      ...
      description: 'Ontdek hoe DGSkills past in jouw schoolarchitectuur. Alles over SSO (Microsoft/Google/SURFconext), privacy-maatregelen, LVS-koppelingen en technische support.',
      canonical: 'https://dgskills.app/ict',
      ogTitle: 'ICT & Informatiemanagers — Veiligheid, Privacy & Integratie | DGSkills',
      ogDescription: 'Veilige en beheersbare digitale geletterdheid voor het VO. Privacy by design en naadloze integratie.',
    ...
    '/ict/privacy': {
      ...
      description: 'Gedocumenteerde privacy-maatregelen (DPIA, EU-data), data opslag in de EU en transparantie over datastromen.',
    ...
    '/ai-geletterdheid-onderwijs-ai-act': {
      ...
      description: 'Sinds februari 2025 is AI-geletterdheid verplicht onder de EU AI Act. Ontdek hoe DGSkills jouw school helpt met veilige AI-missies en gedocumenteerde privacy-maatregelen (DPIA, EU-data).',
...
function getSitemapChangefreq(route) {
  if (route === '/' || route === '/scholen') return 'weekly';
  if (route.startsWith('/gids/')) return 'monthly';
  if (route.startsWith('/compliance/') || route.startsWith('/ict/')) return 'monthly';
  if (route === '/digitale-geletterdheid-vo' || route === '/slo-kerndoelen-digitale-geletterdheid' || route === '/ai-geletterdheid-onderwijs-ai-act') return 'weekly';
  return 'monthly';
}

function getSitemapPriority(route) {
  if (route === '/') return '1.0';
  if (route === '/scholen') return '0.9';
  if (route === '/ict' || route === '/compliance-hub') return '0.8';
  if (route.startsWith('/gids/')) return '0.6';
  return '0.7';
}
...
function writeSeoFiles() {
  const sitemapEntries = SITEMAP_ROUTES.map((route) => {
    ...
    <changefreq>${getSitemapChangefreq(route)}</changefreq>
...
`;
  ...
  const robotsTxt = `# Auto-generated by scripts/prerender.mjs on ${BUILD_DATE}
User-agent: *
Allow: /
...
`;
  ...
  console.log('  ✓ /sitemap.xml en /robots.txt bijgewerkt');
}
