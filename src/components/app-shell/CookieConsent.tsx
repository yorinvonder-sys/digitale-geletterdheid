...
export const CookieConsent: React.FC<CookieConsentProps> = ({ onAccept, onDecline, schoolId }) => {
    ...
    return (
        <div className="fixed bottom-3 left-3 right-3 md:bottom-4 md:left-auto md:right-4 md:max-w-xs z-[60] animate-fade-in-up">
            <div className="bg-duck-bg rounded-[1.5rem] shadow-[2px_4px_24px_rgba(32,32,35,0.15)] border border-duck-ink/10 overflow-hidden">
                {/* Main Banner */}
                <div className="p-3.5 md:p-4">
                    <div className="flex items-start gap-2.5 md:gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <h2 className="text-sm md:text-base font-extrabold text-duck-ink mb-0.5 md:mb-1">Cookies & Privacy</h2>
                            <p className="text-xs md:text-sm text-duck-ink/60 leading-snug md:leading-relaxed">
                                ...
                                <a href="/ict/privacy/cookies" className="text-duck-ink underline underline-offset-2 hover:text-duck-ink/60 transition-colors ml-1">Lees ons cookiebeleid</a>.
                            </p>

                            {/* Expandable Details */}
                            {showDetails && (
                                <div className="mt-3 p-3 bg-duck-bgLight rounded-[1rem] text-sm space-y-2 border border-duck-ink/10">
                                    <p className="font-extrabold text-duck-ink">Welke cookies gebruiken wij?</p>
                                    <ul className="space-y-1.5 text-duck-ink/60">
                                        <li className="flex items-center gap-2">
                                            <IconCheck size={14} className="text-duck-ink" />
                                            <span><strong>Essentieel</strong> – Nodig voor inloggen en authenticatie</span>
                                        </li>
                                        <li className="flex items-center gap-2">
                                            <IconSettings size={14} className="text-duck-ink/40" />
                                            <span><strong>Analytics</strong> – Interne klik-analyse voor productverbetering</span>
                                        </li>
                                    </ul>
                                    <p className="text-xs text-duck-ink/40 mt-2">
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 mt-3 md:mt-4">
                        <button
                            ...
                            className="col-span-2 sm:col-span-1 text-xs md:text-sm text-duck-ink/50 hover:text-duck-ink underline underline-offset-2 order-3 sm:order-1 transition-colors"
                        >
                        ...
                        <button
                            ...
                            className="px-3 py-2 md:px-4 md:py-2.5 text-xs md:text-sm font-extrabold text-duck-ink bg-duck-bgLight border border-duck-ink/20 hover:border-duck-ink rounded-full transition-all duration-300 hover:-translate-y-0.5 order-2 sm:order-3"
                        >
                            Alleen essentieel
                        </button>
                        <button
                            onClick={handleAccept}
                            className="px-3 py-2 md:px-4 md:py-2.5 text-xs md:text-sm font-extrabold text-duck-ink bg-duck-acid border border-duck-ink hover:bg-duck-ink hover:text-duck-acid rounded-full transition-all duration-300 hover:-translate-y-0.5 order-1 sm:order-4"
                        >
                            Alles accepteren
                        </button>
                    </div>
