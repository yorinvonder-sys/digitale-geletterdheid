        ...
        <>
            ...
            {!isVisible && (
                ...
                >
                    ...
                    {isLocked && (
                        <div className="absolute top-0 right-0 w-3.5 h-3.5 bg-lab-coral rounded-full border-2 border-white z-10" />
                    )}
                </button>
            )}
            ...
            {isVisible && (
                <div className="fixed bottom-20 sm:bottom-6 right-2 sm:right-6 w-[94vw] sm:w-[90vw] max-w-[384px] h-[60vh] sm:h-[70vh] max-h-[500px] rounded-2xl shadow-2xl flex flex-col z-[70] overflow-hidden animate-in fade-in slide-in-from-bottom-10 duration-300" style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E7D8BD' }}>
                    ...
                    <div className="p-4 flex items-center justify-between" style={{ background: isLocked ? '#FEF2F2' : 'linear-gradient(to right, #D97848, #D97848)' }}>
                        ...
                        <button
                            onClick={handleClose}
                            className={`p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full hover:bg-white/20 transition-all duration-300 ${isLocked ? 'text-lab-coral hover:text-lab-coral' : 'text-white'}`}
                        >
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    {/* Messages Area */}
                    <div className="flex-1 overflow-y-auto p-4 space-y-4" style={{ backgroundColor: '#FCF6EA' }}>
                        {/* AI Disclaimer Banner */}
                        {messages.length === 0 && (
                            <div className="rounded-xl p-3 text-xs" style={{ backgroundColor: '#D97848' + '10', borderWidth: 1, borderColor: '#D97848' + '25', color: '#D97848' }}>
                                <span className="font-bold">AI Coach</span> is een AI-assistent. Antwoorden zijn
                        ...
                        {messages.map((msg, idx) => (
                            ...
                            >
                                <div
                                    ...
                                    style={msg.role === 'user'
                                        ? { backgroundColor: '#D97848', color: '#FFFFFF', borderTopRightRadius: 0 }
                                        : { backgroundColor: '#FFFFFF', color: '#445865', borderWidth: 1, borderColor: '#E7D8BD', borderTopLeftRadius: 0 }
                                    }
                                >
                                    {msg.role === 'model' ? (
                        ...
                        {isLoading && (
                            <div className="flex justify-start">
                                <div className="p-3 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-2" style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E7D8BD' }}>
                                    <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: '#D97848' }} />
                                    <div className="w-2 h-2 rounded-full animate-pulse delay-100" style={{ backgroundColor: '#D97848' }} />
                                    <div className="w-2 h-2 rounded-full animate-pulse delay-200" style={{ backgroundColor: '#D97848' }} />
                                </div>
                            </div>
                        )}

                        {!isLocked && messages.length <= 1 && (
                            <div className="rounded-2xl p-3" style={{ backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E7D8BD' }}>
                                <p className="text-[11px] font-bold uppercase tracking-widest mb-2" style={{ color: '#D97848' }}>
                                    {quickPromptLabel}
                                </p>
                                <div className="flex flex-wrap gap-2">
                                    {quickPrompts.map((prompt, idx) => (
                                        <button
                                            ...
                                            style={{ backgroundColor: '#FCF6EA', borderWidth: 1, borderColor: '#E7D8BD', color: '#445865' }}
                                            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#D97848'; e.currentTarget.style.color = '#FFFFFF'; e.currentTarget.style.borderColor = '#D97848'; }}
                                            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#FCF6EA'; e.currentTarget.style.color = '#445865'; e.currentTarget.style.borderColor = '#E7D8BD'; }}
                                        >
                                            {prompt}
                                        </button>
                                    ))}
                    ...
                    {/* Input Area */}
                    <div className="p-3 keyboard-safe" style={{ backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#E7D8BD' }}>
                        {isLocked ? (
                            <div className="text-center p-3 text-lab-muted text-sm bg-lab-coral rounded-2xl border border-lab-coral">
