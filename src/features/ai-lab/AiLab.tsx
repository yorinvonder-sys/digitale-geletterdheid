...
import { AgentSelector } from '@/features/ai-lab/AgentSelector';
...
const GamesSection = lazy(() => import('@/features/games/GamesSection').then(m => ({ default: m.GamesSection })));
const AssessmentEngine = lazy(() => import('@/features/assessment/AssessmentEngine').then(m => ({ default: m.AssessmentEngine })));
...
const WordSimulator = lazy(() => import('@/features/word-simulator/WordSimulator').then(m => ({ default: m.WordSimulator })));
const PitchPoliceMission = lazy(() => import('@/features/missions/review/PitchPoliceMission').then(m => ({ default: m.PitchPoliceMission })));
const DataVerzamelaarPreview = lazy(() => import('@/features/ai-lab/previews/DataVerzamelaarPreview'));
const DataJournalistPreview = lazy(() => import('@/features/ai-lab/previews/DataJournalistPreview'));
const FactcheckerPreview = lazy(() => import('@/features/ai-lab/previews/FactcheckerPreview'));
const ApiVerkennerPreview = lazy(() => import('@/features/ai-lab/previews/ApiVerkennerPreview'));
const DashboardDesignerPreview = lazy(() => import('@/features/ai-lab/previews/DashboardDesignerPreview'));
const AiBiasDetectivePreview = lazy(() => import('@/features/ai-lab/previews/AiBiasDetectivePreview'));
...
export const AiLab: React.FC<AiLabProps> = ({ user, onExit, saveProgress, initialRole, libraryData, vsoProfile, devPreviewMode = false }) => {
  ...
  const {
    ...
    initialProgress,
    skipLoading: devPreviewMode || !!sharedData || !!libraryData,
    cloudSyncDisabled: devPreviewMode
  });
  ...
  */
  ...
  const useWideMissionCanvas = Boolean(selectedRole && missionStarted);
  const compactChatRole = selectedRole?.id === 'game-programmeur' || selectedRole?.id === 'ai-trainer';
  const chatWidthClass = showRightPanel
    ? compactChatRole
      ? 'w-full md:w-[34%] xl:w-[32%]'
      : 'w-full md:w-[40%]'
    : 'w-full flex-1';
  ...
  return (
    <div className="fixed inset-0 overflow-hidden flex flex-col items-center pt-safe pb-safe pl-safe pr-safe">
      ...
      <nav className={`w-full ${useWideMissionCanvas ? 'max-w-none' : 'max-w-7xl'} flex justify-between items-center px-2 md:px-4 h-12 md:h-14 shrink-0 print:hidden`}>
        <button
          onClick={() => {
      ...
      <main className={`w-full ${useWideMissionCanvas ? 'max-w-none' : 'max-w-7xl'} flex-1 flex flex-col min-h-0 overflow-hidden relative pb-2 px-2 md:px-4`}>
        {/* Games Section */}
        {/* Games Section */}
        {view === 'games' && (
          ...
          (hasAssessment(selectedRole.id) || selectedRole.id === 'ai-tekengame' || selectedRole.id === 'chatbot-trainer' || selectedRole.id === 'ai-beleid-brainstorm' || selectedRole.id === 'data-detective' || selectedRole.id === 'deepfake-detector' || selectedRole.id === 'filter-bubble-breaker' || selectedRole.id === 'datalekken-rampenplan' || selectedRole.id === 'data-voor-data' || selectedRole.id === 'access-control-engineer') ? (
            ...
            <div className={`flex-1 flex flex-col ${showRightPanel ? 'md:flex-row ipad-stack' : ''} gap-3 h-full min-h-0 pb-1 animate-in fade-in slide-in-from-right-4 duration-500`}>
              ...
              <section className={`chat-column ${selectedRole.id === 'game-programmeur' ? 'game-programmeur-chat' : ''} ${selectedRole.id === 'ai-trainer' ? 'ai-trainer-chat' : ''} flex flex-col bg-white rounded-2xl shadow-sm border border-duck-ink/15 overflow-hidden min-h-0 h-full max-h-full ${chatWidthClass} print:hidden`}>
                ...
                <div className="chat-scroll flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4 min-h-0">
                ...
                <div className="p-3 border-t border-duck-ink/15 relative keyboard-safe">
                  ...
                  {suggestions.length > 0 && !isLoading && (
                    <div className="flex flex-wrap gap-1.5 mb-2 animate-in slide-in-from-bottom-2">
                      <div
                        className="flex items-center gap-1 text-[11px] font-bold text-white mr-1 px-1.5 py-0.5 rounded-md"
                      ...
                      {suggestions.slice(0, 3).map((suggestion, idx) => (
                        ...
                        >
                          ...
                          <span
                            className="text-[9px] font-bold text-white px-1 py-0.5 rounded flex-shrink-0 whitespace-nowrap"
                ...
                <section className="preview-column flex-1 h-full min-h-0 min-w-0 flex flex-col animate-in slide-in-from-right-8 duration-500">
                  <div className={`flex-1 rounded-2xl overflow-hidden relative h-full w-full ${selectedRole?.id === 'verhalen-ontwerper' ? 'ambient-glow border-4 border-duck-acid/50 shadow-inner' : 'bg-duck-ink border-4 border-duck-ink/15 shadow-2xl'}`}>
                    ...
                    {selectedRole?.id === 'game-programmeur' && (activeGameCode || selectedRole.initialCode) ? (
                      <GamePreview
                        code={activeGameCode || selectedRole.initialCode || ''}
                        autoStart={false}
                        isGenerating={isLoading}
                        missionId="game-programmeur"
                        completedSteps={completedSteps}
                        initialCode={selectedRole.initialCode}
                        user={user ? {
                          uid: user.uid,
                          displayName: user.displayName || 'Student',
                          studentClass: user.studentClass,
