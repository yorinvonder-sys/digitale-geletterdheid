...
const BLOCKED_WORDS = [
    ...
    'kech', 'kansen', 'flikker', 'mongool', 'debiel', 'idioot', 'achterlijk', 'retard',
...
export const AiBeleidBrainstormPreview: React.FC<AiBeleidBrainstormPreviewProps> = ({ user, onComplete }) => {
    ...
    const handleSubmitIdee = async () => {
        ...
        setSubmitting(true);
        const saved = await submitAiBeleidIdee({
            uid: user.uid,
        ...
        if (saved) {
            ...
            setSubmitError('Je idee kon niet worden opgeslagen. Controleer je verbinding en probeer het opnieuw.');
        }
        setSubmitting(false);
    };
    ...
    const handleVote = async (ideeId: string) => {
        ...
        if (success) {
            ...
            setIdeeen(prev => prev.map(i =>
                i.id === ideeId ? {
                    ...i,
                    stemmen: (i.stemmen || 0) + 1,
                    gestemdeUids: [...(i.gestemdeUids || []), user.uid],
                } : i
            ));
        }
    };
    ...
    const handleSurveySubmit = async () => {
        ...
        try {
            const success = await submitAiBeleidSurvey({
                uid: user.uid,
                studentName: user.displayName || 'Anoniem',
                studentClass: user.studentClass,
                schoolId: user.schoolId,
            ...
            if (success) {
                ...
                setSurveyError('Je antwoorden konden niet worden opgeslagen. Probeer het nog een keer.');
            }
        } catch (error) {
                ...
                <div className="max-w-lg mx-auto">
                    ...
                    <div className={`${selectedCategory.bgColor} ${selectedCategory.borderColor} border-2 rounded-2xl p-6 mb-6`}>
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-bold text-white text-xl">{selectedCategory.label}</h3>
                                <p className="text-white/85">{selectedCategory.prompt}</p>
                            </div>
                        </div>
                    </div>
