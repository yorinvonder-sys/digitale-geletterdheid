...
import { DuelLobby } from '@/features/games/DrawingDuel/DuelLobby';
import { DuelGame } from '@/features/games/DrawingDuel/DuelGame';
...
const INTRO_STEPS = [
    'Je krijgt een woord te zien (bijvoorbeeld "kat" of "huis")',
    'Teken het woord op het tekenveld hieronder',
    'De AI bekijkt je tekening en raadt wat het is',
    'Hoe beter de AI het herkent, hoe meer punten je krijgt!',
];

const INTRO_STEP_INITIAL_DELAY_MS = 800;
const INTRO_STEP_REVEAL_INTERVAL_MS = 1300;
...
export const DrawingGamePreview: React.FC<DrawingGamePreviewProps> = ({ onLevelComplete, onStart, onXPEarned, user, initialState, onSave, enablePeerFeedback, peerFeedbackProps }) => {
    ...
    useEffect(() => {
        ...
        const timers = INTRO_STEPS.map((_, index) =>
            window.setTimeout(() => {
                setVisibleIntroSteps(index + 1);
            }, INTRO_STEP_INITIAL_DELAY_MS + index * INTRO_STEP_REVEAL_INTERVAL_MS)
        );
