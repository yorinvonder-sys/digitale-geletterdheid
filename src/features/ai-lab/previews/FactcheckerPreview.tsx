...
const FactcheckerPreview: React.FC = () => {
    ...
    return (
        <div className="w-full h-full bg-gradient-to-br from-lab-gold to-white flex flex-col overflow-hidden">
            {/* Header */}
            <div className="shrink-0 bg-gradient-to-r from-lab-gold to-lab-gold px-4 py-3 flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                ...
                <div className="bg-white rounded-xl border-2 border-lab-gold overflow-hidden shadow-sm">
                    ...
                    <div className="p-4">
                        <h3 className="font-black text-lab-ink text-sm leading-tight">
                            "Scholen gaan per 2027 over op vierdaagse schoolweek"
                        </h3>
                        <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-lab-muted">
                            <span className="bg-lab-cream px-2 py-0.5 rounded-full">📅 15 november 2024</span>
                            <span className="bg-lab-cream px-2 py-0.5 rounded-full">🌐 nieuwsflits247.nl</span>
                            <span className="bg-lab-cream px-2 py-0.5 rounded-full">👤 Auteur onbekend</span>
                        </div>
                ...
                <div className="bg-white rounded-xl border border-lab-gold shadow-sm overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] bg-lab-gold text-lab-ink px-2 py-0.5 rounded-full font-bold">
                                {filledCount}/5
                            </span>
                        </div>
                                ...
                                <div key={i} className="px-4 py-3">
                                    ...
                                    <div className="flex gap-1.5 pl-8">
                                        <button
                                            ...
                                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${ratings[i] === 'ok' ? 'bg-lab-sage text-white ring-2 ring-lab-sage' : 'bg-lab-cream text-lab-muted hover:bg-lab-sage hover:text-white'}`}
                                        >
                                            <CheckCircle2 size={10} /> Oké
                                        </button>
                                        <button
                                            onClick={() => setRating(i, 'twijfel')}
                                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${ratings[i] === 'twijfel' ? 'bg-lab-gold text-lab-ink ring-2 ring-lab-gold' : 'bg-lab-cream text-lab-muted hover:bg-lab-gold hover:text-lab-ink'}`}
                                        >
                                            <HelpCircle size={10} /> Twijfel
                                        </button>
                                        <button
                                            onClick={() => setRating(i, 'nep')}
                                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${ratings[i] === 'nep' ? 'bg-lab-coral text-white ring-2 ring-lab-coral' : 'bg-lab-cream text-lab-muted hover:bg-lab-coral hover:text-white'}`}
                                        >
