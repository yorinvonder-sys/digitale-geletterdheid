        ...
        <div className="flex flex-col h-full bg-lab-bg text-lab-dark p-4">
            ...
            <div className="flex-1 flex gap-4 min-h-0">
                ...
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    {/* Solution Area */}
                    <div className="flex-1 bg-white rounded-xl p-4 border border-lab-line shadow-sm flex flex-col">
                        <h4 className="font-bold text-lab-textLight mb-2 uppercase text-xs tracking-wider">Jouw Oplossing (Sleep / Klik):</h4>

                        <div className="flex-1 space-y-2">
                            {selectedSteps.length === 0 && (
                                <div className="h-full flex items-center justify-center text-lab-textLight text-sm border-2 border-dashed border-lab-line rounded-xl">
                            ...
                            {selectedSteps.map((step, index) => (
                                <div key={step.id} className="flex items-center gap-3 animate-in fade-in slide-in-from-left-4">
                                    ...
                                    <button
                                        ...
                                        disabled={!!result}
                                        className="flex-1 bg-lab-primary/10 border border-lab-primary/30 hover:bg-lab-coral hover:border-lab-coral hover:text-lab-coral text-lab-dark text-left px-4 py-3 rounded-xl font-medium text-sm transition-colors shadow-sm group"
                                    >
                                    ...
                                    {index < selectedSteps.length - 1 && (
                                        <div className="h-4 border-l-2 border-dashed border-lab-line absolute ml-3 mt-10 -z-0" />
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-white rounded-xl p-4 border border-lab-line shadow-sm min-h-[140px]">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {availableSteps.map(step => (
                                <button
                                    ...
                                    disabled={!!result}
                                    className="px-3 py-2 bg-lab-bg hover:bg-lab-primary/10 text-lab-text text-xs font-bold rounded-lg border border-lab-line hover:border-lab-primary/40 transition-colors shadow-sm active:scale-95"
                                >
                                    + {step.text}
                                </button>
                            ))}
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="px-4 py-3 bg-white border border-lab-line text-lab-text font-bold rounded-xl hover:bg-lab-cream disabled:opacity-50 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center shadow-sm"
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${result === 'success' ? 'bg-lab-coral text-white' : result === 'fail' ? 'bg-lab-coral text-white' : 'bg-lab-primary text-white hover:bg-lab-primaryDark'}
                            `}
                        >
                            {result === 'success' ? (
                    ...
                    {result === 'fail' && (
                        <div className="bg-lab-coral text-white px-4 py-2 rounded-xl text-center text-sm border border-lab-coral animate-in shake motion-reduce:animate-none">
