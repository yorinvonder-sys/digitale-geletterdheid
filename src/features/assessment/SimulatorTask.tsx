...
export const SimulatorTask: React.FC<Props> = ({ task, onComplete }) => {
    ...
    const getIcon = (iconName?: string) => {
        switch (iconName) {
            case 'FileText': return <FileText size={32} className="text-lab-muted" />;
            case 'Image': return <Image size={32} className="text-lab-muted" />;
    ...
    return (
        <div className="flex flex-col h-full bg-lab-bg text-lab-dark p-4">
            ...
            {/* Desktop Area */}
            <div className="flex-1 bg-white rounded-2xl border-2 border-lab-line shadow-md relative overflow-hidden flex">
                ...
                <div className="w-2/5 sm:w-1/3 bg-lab-bg/50 border-l border-lab-line p-2 sm:p-4 flex flex-col gap-4 justify-center">
                    {task.targets.map(target => (
                        <div
                            ...
                            className={`
                                ...
                                ${target.type === 'trash' ? 'border-lab-coral/50 bg-lab-coral hover:bg-lab-coral hover:text-white' : 'border-lab-accent/50 bg-lab-accent/5 hover:bg-lab-accent/10'}
                            `}
                        >
                            {target.type === 'trash' ? <Trash2 size={32} className="text-lab-coral" /> : <UploadCloud size={32} className="text-lab-accent" />}
                ...
                {feedback && (
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white border border-lab-line px-6 py-3 rounded-xl shadow-xl font-bold text-lab-dark animate-in slide-in-from-bottom-4">
                        {feedback}
                    </div>
                )}
            </div>

            <div className="mt-4 bg-white p-4 rounded-xl border border-lab-line shadow-sm text-center">
