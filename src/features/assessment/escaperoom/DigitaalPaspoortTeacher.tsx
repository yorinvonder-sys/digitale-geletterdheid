...
function scoreBarColor(score: number): string {
  if (score >= 80) return 'bg-lab-coral';
  if (score >= 50) return 'bg-lab-coral';
  return 'bg-lab-coral';
}
...
function niveauBadge(niveau: NulmetingResult['niveau']): { label: string; className: string } {
  switch (niveau) {
    case 'gevorderd':
      return { label: 'Gevorderd', className: 'bg-lab-sage text-white border-lab-sage' };
    case 'basis':
      return { label: 'Basis', className: 'bg-lab-teal text-white border-lab-teal' };
    case 'starter':
    default:
      return { label: 'Starter', className: 'bg-lab-teal text-white border-lab-teal' };
  }
}
...
export const DigitaalPaspoortTeacher: React.FC<DigitaalPaspoortTeacherProps> = ({ klasResults }) => {
  ...
  if (klasResults.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-lab-line p-8 text-center">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-lab-line shadow-sm p-6">
        <h3 className="text-sm font-black text-lab-ink uppercase tracking-widest mb-4 flex items-center gap-2">
          <div className="w-2 h-2 bg-lab-coral rounded-full" />
      ...
      {/* Leerlingen tabel */}
      <div className="bg-white rounded-2xl border border-lab-line shadow-sm overflow-hidden">
        <div className="p-4 border-b border-lab-line">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-lab-line">
                <th className="text-left px-4 py-3">
                  ...
                  >
                    Naam
                    <ArrowUpDown size={10} className={sortField === 'name' ? 'text-lab-muted' : ''} />
                  </button>
                </th>
                <th className="text-center px-2 py-3">
                  ...
                  >
                    Niveau
                    <ArrowUpDown size={10} className={sortField === 'score' ? 'text-lab-muted' : ''} />
                  </button>
                </th>
                {DOMEINEN.map(d => (
            ...
            <tbody>
              {sortedResults.map(({ studentName, studentId, result }) => {
                ...
                return (
                  <tr key={studentId} className="border-b border-lab-line hover:bg-lab-cream transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-bold text-lab-ink">{studentName}</span>
                    </td>
                      ...
                      return (
                        <td key={d.key} className="px-2 py-3">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-2 bg-lab-cream rounded-full overflow-hidden min-w-[40px]">
                              <div
      ...
      {/* Aanbeveling */}
      {zwaksteDomein && (
        <div className="bg-lab-coral border border-lab-coral rounded-2xl p-5">
          <h4 className="text-xs font-black text-lab-coral uppercase tracking-widest mb-2">Aanbeveling</h4>
