...
function getNiveauKleur(niveau: 'starter' | 'basis' | 'gevorderd'): string {
  switch (niveau) {
    case 'gevorderd': return 'bg-lab-coral/20 text-lab-sage border-lab-coral/40';
    case 'basis':     return 'bg-lab-coral/20 text-lab-teal border-lab-coral/40';
    case 'starter':   return 'bg-lab-coral/20 text-lab-gold border-lab-coral/40';
  }
}
...
function getDomeinBarKleur(key: DomeinKey): { bar: string; text: string; badge: string } {
  ...
  const map: Record<string, { bar: string; text: string; badge: string }> = {
    indigo:  { bar: 'bg-lab-coral',  text: 'text-lab-coral',  badge: 'bg-lab-coral/20 text-lab-coral' },
    emerald: { bar: 'bg-lab-coral', text: 'text-lab-sage', badge: 'bg-lab-coral/20 text-lab-sage' },
    violet:  { bar: 'bg-lab-coral',  text: 'text-lab-teal',  badge: 'bg-lab-coral/20 text-lab-teal' },
    rose:    { bar: 'bg-lab-coral',    text: 'text-lab-coral',    badge: 'bg-lab-coral/20 text-lab-coral' },
    sky:     { bar: 'bg-lab-coral',     text: 'text-lab-teal',     badge: 'bg-lab-coral/20 text-lab-teal' },
  };
  return map[kleur] ?? { bar: 'bg-lab-coral', text: 'text-lab-muted', badge: 'bg-lab-coral/20 text-lab-muted' };
}
...
export const GroeiPaspoort: React.FC<GroeiPaspoortProps> = ({
  ...
  return (
    <div className="min-h-screen bg-lab-ink flex items-start justify-center p-4 py-10">
      <motion.div
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center space-y-3">
          ...
          <div
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm ${
              groeiPositief
                ? 'bg-lab-coral/15 text-lab-sage'
                : groeiNeutraal
                ? 'bg-lab-muted/60 text-lab-muted'
        ...
        <motion.div
          ...
          className="bg-lab-ink/50 border border-lab-line/50 rounded-2xl p-5 flex items-center justify-center gap-4"
        >
          <span
          ...
          <motion.div
            ...
            className="flex items-center text-lab-muted"
          >
            <ArrowRight size={20} />
          </motion.div>
          <span
          ...
          {DOMEINEN.map((domein, index) => {
            ...
            return (
              <motion.div
                ...
                className="bg-lab-ink/50 border border-lab-line/50 rounded-xl p-4 space-y-2"
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-lab-muted w-10 text-right shrink-0">{Math.round(data.nulmeting)}</span>
                  <div className="relative flex-1 h-2 bg-lab-muted rounded-full overflow-hidden">
                    {/* Nulmeting bar */}
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${data.nulmeting}%` }}
                      transition={{ delay: 0.4 + index * 0.1, duration: 0.6, ease: 'easeOut' }}
                      className="absolute inset-y-0 left-0 bg-lab-muted rounded-full"
                    />
                    {/* Eindmeting bar */}
                    <motion.div
                      initial={{ width: 0 }}
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3">
          <div className="bg-lab-ink/50 border border-lab-coral/30 rounded-2xl p-4 space-y-2">
        ...
        <motion.div
          variants={itemVariants}
          className="border border-dashed border-lab-line rounded-2xl p-5 space-y-2"
        >
          <div className="flex items-center gap-2">
            <Star size={14} className="text-lab-muted" />
            <span className="text-xs font-bold text-lab-muted uppercase tracking-wide">
          ...
          <p className="text-[10px] text-lab-muted mt-1">Gegenereerd door AI</p>
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <button
            onClick={onContinue}
            className="w-full flex items-center justify-center gap-2 py-4 bg-lab-coral hover:bg-lab-coral hover:text-white active:scale-[0.98] text-white font-black rounded-2xl text-sm transition-all shadow-lg shadow-lab-coral/40"
          >
            Ga verder
            <ArrowRight size={18} />
          </button>
