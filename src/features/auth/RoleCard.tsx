      ...
      <div className={`relative w-full h-full duration-500 preserve-3d`}>
        ...
        <div 
          ...
          className={`
            ...
            ${isSelected 
              ...
              : 'shadow-md border-lab-line hover:shadow-xl'
            }
        ...
        >
          ...
          <div className="absolute inset-0 p-6 flex flex-col items-center justify-center transition-opacity duration-300 group-hover:opacity-0">
             <div className={`text-4xl mb-4 p-4 rounded-2xl bg-lab-cream text-lab-muted shadow-inner`}>
                {role.icon}
             </div>
             <h3 className="font-extrabold text-xl mb-2 text-lab-ink">{role.title}</h3>
             <p className="text-sm text-lab-muted font-medium line-clamp-2">
                {role.description}
             </p>
             <div className="mt-4 px-3 py-1 bg-lab-cream text-lab-muted rounded-full text-xs font-bold uppercase tracking-wider">
                Hover voor voorbeeld
             </div>
          </div>

          {/* HOVER STATE (Visual Preview) - Fades in on hover */}
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-lab-cream">
             {/* The Visual Component */}
             <div className="w-full h-full relative">
                {role.visualPreview}
                
                {/* Overlay with CTA */}
                <div className="absolute inset-0 bg-gradient-to-t from-lab-ink/80 via-transparent to-transparent flex flex-col justify-end p-6">
        ...
        <div 
          className={`
            absolute inset-0 w-full h-full backface-hidden rounded-[2rem] p-6 bg-lab-ink text-white flex flex-col shadow-xl border border-lab-line
        ...
        >
          ...
          <button 
            ...
            className="mt-auto w-full py-3 bg-lab-primary hover:bg-lab-primaryDark rounded-xl font-bold flex items-center justify-center gap-2 transition-colors shadow-lg shadow-lab-coral/20"
          >
