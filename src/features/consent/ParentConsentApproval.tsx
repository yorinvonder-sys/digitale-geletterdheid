...
export function ParentConsentApproval() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(217, 120, 72,0.12),_transparent_45%),linear-gradient(180deg,#FCF6EA_0%,#FFFFFF_100%)] px-4 py-10">
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-[2rem] border border-lab-line shadow-xl shadow-lab-line/40 p-8 md:p-10">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 ${
            isApproved ? 'bg-lab-sage text-white' : 'bg-lab-coral text-white'
          }`}>
            {loading ? <Loader2 className="animate-spin" size={30} /> : isApproved ? <CheckCircle2 size={30} /> : <ShieldCheck size={30} />}
          </div>

          <h1 className="text-3xl font-black text-lab-ink mb-3">
          ...
          {!loading && error && (
            <div className="rounded-2xl border border-lab-coral bg-lab-coral p-4 flex items-start gap-3">
              <MailWarning size={18} className="text-lab-coral mt-0.5 shrink-0" />
              <p className="text-sm text-lab-coral leading-relaxed">{error}</p>
            </div>
          )}
          ...
          {!loading && preview && (
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-lab-line bg-lab-cream p-5">
                ...
                <div className="space-y-3">
                  {preview.consentTypes.map((consentType) => (
                    <div key={consentType} className="rounded-xl bg-white border border-lab-line px-4 py-3">
                      <p className="text-sm font-semibold text-lab-ink">{formatConsentLabel(consentType)}</p>
              ...
              <div className="rounded-2xl border border-lab-coral bg-lab-coral p-4">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-lab-coral text-white px-5 py-4 text-sm font-bold hover:bg-lab-coral hover:text-white disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
              >
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-lab-line bg-lab-cream px-4 py-3">
