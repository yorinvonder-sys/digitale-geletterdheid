    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                {/* Sub-tabs */}
                <div className="flex bg-white border border-lab-line rounded-2xl p-1 gap-1 overflow-x-auto">
                    {SUB_TABS.map(tab => (
                        <button
                            key={tab.id}
                            ...
                            className={`flex items-center gap-2 px-2.5 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all ${activeTab === tab.id
                                ? 'bg-lab-coral text-white shadow-sm'
                                : 'text-lab-muted hover:text-lab-ink hover:bg-lab-cream'
                                }`}
                        >
                            {tab.icon}
                            {tab.label}
                ...
                <div className="relative ml-auto">
                    <select
                        ...
                        className="appearance-none pl-4 pr-8 py-2 bg-white border border-lab-line rounded-xl text-sm font-bold text-lab-muted focus:outline-none focus:ring-2 focus:ring-lab-coral cursor-pointer"
                    >
                        {YEAR_OPTIONS.map(y => (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                <div className="bg-white rounded-2xl border border-lab-line px-4 py-3 text-center">
                    <p className="text-[9px] font-black text-lab-muted uppercase tracking-widest">Transacties</p>
                    <p className="text-xl font-black text-lab-ink mt-0.5">{transactions.length}</p>
                </div>
                <div className="bg-white rounded-2xl border border-lab-line px-4 py-3 text-center">
                    <p className="text-[9px] font-black text-lab-muted uppercase tracking-widest">Bonnetjes</p>
                    <p className="text-xl font-black text-lab-ink mt-0.5">{receipts.length}</p>
                </div>
                <div className="bg-white rounded-2xl border border-lab-line px-4 py-3 text-center">
                    <p className="text-[9px] font-black text-lab-muted uppercase tracking-widest">Facturen</p>
                    <p className="text-xl font-black text-lab-ink mt-0.5">{invoices.length}</p>
                </div>
                <div className={`rounded-2xl border px-4 py-3 text-center ${summary.profit >= 0 ? 'bg-lab-sage border-lab-sage' : 'bg-lab-coral border-lab-coral'}`}>
                    <p className="text-[9px] font-black text-lab-muted uppercase tracking-widest">Winst {year}</p>
                    <p className={`text-xl font-black mt-0.5 ${summary.profit >= 0 ? 'text-lab-sage' : 'text-lab-coral'}`}>
