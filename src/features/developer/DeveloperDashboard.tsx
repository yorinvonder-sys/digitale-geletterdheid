...
const MissionQualityDashboard = React.lazy(() => import('./mission-quality/MissionQualityDashboard').then(m => ({ default: m.MissionQualityDashboard })));
import {
...
type TabId = 'overview' | 'tasks' | 'timeline' | 'docs' | 'analytics' | 'boekhouding' | 'sprintplan' | 'notities' | 'reviewChecklist' | 'missionQuality' | 'dpaGenerator';
...
export function DeveloperDashboard({ user, onLogout, onSwitchView }: DeveloperDashboardProps) {
    ...
    const tabs = [
        ...
        { id: 'missionQuality', label: 'Missiekwaliteit', icon: ScanSearch },
        { id: 'reviewChecklist', label: 'Review checklist', icon: ClipboardCheck },
    ...
    const renderTabContent = () => {
        switch (activeTab) {
                                ...
                                <div className="relative z-10">
                                    ...
                                    {recentReflection ? (
                                        <div className="space-y-6">
                                            <div className="p-6 bg-white/10 rounded-2xl border border-white/15 italic text-white/80 text-sm leading-relaxed">
                                                "{recentReflection.evidence?.reflection}"
                                                <div className="mt-4 flex items-center gap-2 not-italic">
                                                    <span className="text-[10px] font-black text-white/65 uppercase">Bron:</span>
                                                    <span className="text-[10px] font-bold text-white uppercase">{recentReflection.title}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="py-10 text-center">
                                            <p className="text-white/75 italic text-sm">Nog geen reflecties beschikbaar. Voeg bewijs toe aan voltooide taken om coach-inzichten te ontvangen.</p>
                                        </div>
                                    )}
                                </div>
            ...
            case 'missionQuality':
                return <Suspense fallback={<TabLoader />}><MissionQualityDashboard userId={user.uid} /></Suspense>;
            case 'dpaGenerator':
