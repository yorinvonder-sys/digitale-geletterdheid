        ...
        <div className="space-y-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-duck-bg rounded-xl flex items-center justify-center">
                            <Settings size={20} className="text-duck-ink/60" />
                        </div>
                        <h3 className="text-lg font-black text-duck-ink uppercase tracking-tight">Bedrijfsgegevens</h3>
                    </div>
                    <button
                        onClick={() => setShowSettings(!showSettings)}
                        className="text-xs font-bold text-duck-ink uppercase tracking-widest"
                    >
                ...
                {showSettings ? (
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-black text-duck-ink/60 uppercase tracking-widest mb-1">Bedrijfsnaam</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-duck-ink/60 uppercase tracking-widest mb-1">KvK-nummer</label>
                                <input
                                    type="text"
                        ...
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                className="w-4 h-4 text-duck-acid rounded"
                            />
                            <label htmlFor="starter" className="text-sm font-medium text-duck-ink/60">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-duck-acid text-duck-ink rounded-xl text-sm font-bold disabled:opacity-50 transition-colors"
                        >
                            <Save size={16} />
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <div>
                            <p className="text-[10px] text-duck-ink/60 font-black uppercase tracking-widest mb-1">Bedrijfsnaam</p>
                            <p className="font-bold text-duck-ink">{settings?.business_name || '—'}</p>
                        </div>
                        <div>
                            <p className="text-[10px] text-duck-ink/60 font-black uppercase tracking-widest mb-1">KvK-nummer</p>
                            <p className="font-bold text-duck-ink">{settings?.kvk_number || '—'}</p>
                        </div>
                        <div>
                            <p className="text-[10px] text-duck-ink/60 font-black uppercase tracking-widest mb-1">Startersaftrek</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-duck-ink rounded-[2rem] p-8 text-white space-y-1">
                    <h3 className="text-sm font-black uppercase tracking-widest text-white/60 mb-6">
                        IB Berekening {summary.year} — ZZP Box 1
                    </h3>

                    <Row label="Bruto-omzet"         value={tax.grossIncome}         color="text-white" />
                    <Row label="Zakelijke kosten"     value={-tax.totalExpenses}       color="text-white" />
                    <Divider />
                    <Row label="Winst onderneming"    value={tax.profit}               bold color="text-white" />

                    <div className="h-3" />
                    <p className="text-[10px] text-duck-ink/60 font-black uppercase tracking-widest">Aftrekposten</p>
                    ...
                    <div className="h-3" />
                    <p className="text-[10px] text-duck-ink/60 font-black uppercase tracking-widest">Inkomstenbelasting</p>

                    <Row label={`Schijf 1 (t/m €${currentTaxConfig.bracket1Limit.toLocaleString('nl-NL')} × ${(currentTaxConfig.bracket1Rate * 100).toFixed(2)}%)`}
                        value={Math.min(tax.taxableIncome, currentTaxConfig.bracket1Limit) * currentTaxConfig.bracket1Rate} color="text-duck-ink/60" />
                    {tax.taxableIncome > currentTaxConfig.bracket1Limit && (
                        <Row label={`Schijf 2 (boven €${currentTaxConfig.bracket1Limit.toLocaleString('nl-NL')} × ${(currentTaxConfig.bracket2Rate * 100).toFixed(2)}%)`}
                            value={(tax.taxableIncome - currentTaxConfig.bracket1Limit) * currentTaxConfig.bracket2Rate} color="text-duck-ink/60" />
                    )}
                    <Divider />
                    <Row label="Geschatte belasting" value={tax.estimatedTax} bold color="text-duck-ink" />

                    <p className="text-[10px] text-duck-ink/60 pt-2">
                ...
                <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {checklistItems.map((item, i) => (
                                <div key={i} className="flex items-center gap-3">
                                    {item.done ? (
                                        <CheckCircle size={18} className="text-duck-ink/60 shrink-0" />
                                    ) : (
                                        <AlertCircle size={18} className="text-duck-ink shrink-0" />
                                    )}
                                    <span className={`text-sm font-medium ${item.done ? 'text-duck-ink/60' : 'text-duck-ink'}`}>
                                        {item.label}
                                    </span>
                                </div>
                            ))}
                    ...
                    <div className="bg-duck-acid rounded-[2rem] border border-duck-acid p-8">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 bg-duck-ink/10 rounded-xl flex items-center justify-center">
                                <FileText size={20} className="text-duck-ink" />
                            </div>
                            <div>
                                <h3 className="font-black text-duck-ink uppercase tracking-tight">PDF Exporteren</h3>
                                <p className="text-[10px] text-duck-ink font-bold uppercase tracking-widest">Belastingoverzicht {summary.year}</p>
                            </div>
                        </div>
                        ...
                        <button
                            onClick={exportPDF}
                            className="w-full flex items-center justify-center gap-2 py-3 bg-duck-ink text-white rounded-xl font-bold text-sm transition-colors shadow-sm"
                        >
                            <Download size={16} />
                            Download PDF
                        </button>
                    ...
                    <div className="bg-duck-bg border border-duck-ink/15 rounded-2xl px-5 py-4">
                        <p className="text-xs text-duck-ink/60 leading-relaxed">
                            <strong className="text-duck-ink/60">Let op:</strong> Dit overzicht is indicatief en dient als voorbereiding op je aangifte.
                            De definitieve aangifte doe je via <strong className="text-duck-ink/60">Mijn Belastingdienst</strong>.
...
function CopyField({ label, rugnummer, value, hint }: {
    ...
    return (
        <div className="flex items-center gap-3 p-3 bg-white border border-duck-ink/15 rounded-xl hover:border-duck-acid transition-colors group">
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                    {rugnummer && (
                        <span className="text-[9px] font-black text-duck-ink bg-duck-acid/30 px-1.5 py-0.5 rounded uppercase tracking-widest shrink-0">
                            {rugnummer}
                        </span>
                    )}
                    <span className="text-xs text-duck-ink/60 font-medium truncate">{label}</span>
                </div>
                <p className="text-sm font-black text-duck-ink mt-0.5 tabular-nums">{value}</p>
                {hint && <p className="text-[10px] text-duck-ink/60 mt-0.5">{hint}</p>}
            </div>
            <button
                onClick={handleCopy}
                className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-duck-bg group-hover:bg-duck-acid transition-colors"
                title="Kopiëren"
            >
                {copied
                    ? <Check size={14} className="text-duck-ink/60" />
                    : <Copy size={14} className="text-duck-ink/60 group-hover:text-duck-ink" />
                }
            </button>
        </div>
    );
    ...
    return (
        <div className="bg-white rounded-[2rem] border border-duck-ink/15 shadow-sm p-8">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-duck-ink/60 font-bold uppercase tracking-widest">Kopieer waarden direct naar Mijn Belastingdienst</p>
                    </div>
                </div>
                <a
            ...
            <div className="space-y-6">
                {/* Stap 1: Inkomsten */}
                <div>
                    <p className="text-[10px] font-black text-duck-ink/60 uppercase tracking-widest mb-3">
                ...
                {/* Stappenplan */}
                <div className="bg-duck-bg rounded-2xl p-5 space-y-3 border border-duck-ink/15">
                    <p className="text-xs font-black text-duck-ink/60 uppercase tracking-widest">Stappenplan aangifte</p>
                    {[
                        ...
                        <div key={i} className="flex items-start gap-3">
                            <span className="shrink-0 w-5 h-5 rounded-full bg-duck-acid text-duck-ink text-[10px] font-black flex items-center justify-center mt-0.5">
                                {i + 1}
                            </span>
                            <p className="text-xs text-duck-ink/60 leading-relaxed">{step}</p>
                        </div>
                    ))}
                </div>
            </div>
...
function Row({ label, value, bold = false, color = 'text-duck-ink/60' }: {
    ...
    return (
        <div className="flex justify-between items-center py-0.5">
            <span className={`text-xs ${bold ? 'font-black text-white' : 'text-duck-ink/60'}`}>{label}</span>
