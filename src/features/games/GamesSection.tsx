...
import { isGameEnabled, subscribeToPermissions, type GamePermissions } from '@/services/PermissionService';
import { Bomberman } from '@/features/games/Bomberman';
import { DrawingDuelGame } from '@/features/games/DrawingDuelGame';
import { TypingTrainer } from '@/features/games/TypingTrainer';
import { TeacherGameToggle } from '@/features/games/TeacherGameToggle';
...
export const GamesSection: React.FC<GamesSectionProps> = ({
    ...
    useEffect(() => {
        const applyPermissionSnapshot = (perms: GamePermissions) => {
            const isEnabled = (gameId: string) =>
                !perms.blocked_games.includes(gameId)
                && (perms.enabled_games.length === 0 || perms.enabled_games.includes(gameId));
            setPermissions({
                'arena-battle': isEnabled('arena-battle'),
                'code-breaker': isEnabled('code-breaker'),
                'typing-trainer': isEnabled('typing-trainer'),
                'drawing-duel': true,
            });
        };
