...
export const FilterBubbleBreakerMission: React.FC<Props> = ({ onBack, onComplete }) => {
    ...
    const renderFeedCard = (item: FeedItem, isB: boolean) => (
        <div key={item.title} className={`p-3 rounded-2xl border transition-all duration-300 hover:shadow-md ${isB ? 'bg-duck-ink/5 border-duck-ink/20' : 'bg-duck-ink/5 border-duck-ink/20'}`}>
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className={`grid ${activeFeed === 'both' ? 'grid-cols-2' : 'grid-cols-1 max-w-sm mx-auto'} gap-4`}>
                        {(activeFeed === 'both' || activeFeed === 'A') && (
                            <div>
                                <div className="flex items-center gap-2 mb-3 bg-duck-ink/10 border border-duck-ink/20 px-3 py-2 rounded-full"><span>{PROFILE_A.emoji}</span><span className="text-xs font-black text-duck-ink" style={{ fontFamily: "'Outfit', system-ui, sans-serif" }}>Feed van {PROFILE_A.name}</span></div>
                                <div className="space-y-2">{FEED_A.map(item => renderFeedCard(item, false))}</div>
                            </div>
                        )}
                        {(activeFeed === 'both' || activeFeed === 'B') && (
                            <div>
                                <div className="flex items-center gap-2 mb-3 bg-duck-ink/10 border border-duck-ink/20 px-3 py-2 rounded-full"><span>{PROFILE_B.emoji}</span><span className="text-xs font-black text-duck-ink" style={{ fontFamily: "'Outfit', system-ui, sans-serif" }}>Feed van {PROFILE_B.name}</span></div>
                ...
                <div className="max-w-lg mx-auto space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {FEED_A.map(item => (
                                <span key={item.title} className="text-[10px] bg-duck-ink/10 text-duck-ink px-2 py-1 rounded-full border border-duck-ink/20 font-bold">{item.category}</span>
                            ))}
                        </div>
                    </div>
