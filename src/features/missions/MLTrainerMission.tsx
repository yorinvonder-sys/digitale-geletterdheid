  ...
  return (
    <button
      ...
      style={{
        ...
        border: `2px solid ${selected ? '#ff3c21' : '#e3e2dc'}`,
        transform: selected ? 'scale(1.05)' : undefined,
        boxShadow: selected ? '0 10px 25px -5px rgba(217, 120, 72, 0.15)' : undefined
      }}
    >
      {draggable && (
        <GripVertical size={14} className="absolute top-1 right-1" style={{ color: '#e3e2dc' }} />
      )}
      <div className="flex items-center gap-2">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg font-bold`} style={{
          ...
          color: showLabel && item.label
            ? item.label === 'appel' ? '#ff3c21' : '#e1ff01'
            : '#6f6e69'
        }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap gap-1">
            {item.features.map((f, i) => (
              <span key={i} className="text-[10px] px-1.5 py-0.5 rounded-full font-medium" style={{ backgroundColor: '#e3e2dc', color: '#6f6e69', border: '1px solid #E7D8BD' }}>
                {f}
              </span>
            ))}
          </div>
      ...
      {showLabel && item.label && (
        <div className="mt-1.5 text-[10px] font-bold uppercase tracking-wider" style={{
          color: item.label === 'appel' ? '#ff3c21' : '#e1ff01'
        }}>
      ...
      {item.inSet && (
        <div className="absolute -top-1 -right-1 text-[8px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{
          backgroundColor: item.inSet === 'train' ? '#ff3c21' : '#202023'
        }}>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl p-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
        <h3 className="text-sm font-bold mb-3 text-center" style={{ color: '#6f6e69' }}>Confusion Matrix</h3>
        <div className="grid grid-cols-3 gap-1 max-w-xs mx-auto">
          {/* Header */}
          <div />
          <div className="text-center text-[10px] font-bold py-1" style={{ color: '#ff3c21' }}>Voorspeld: Appel</div>
          <div className="text-center text-[10px] font-bold py-1" style={{ color: '#e1ff01' }}>Voorspeld: Banaan</div>

          {/* Row 1: Echt Appel */}
          <div className="text-right text-[10px] font-bold py-3 pr-2" style={{ color: '#ff3c21' }}>Echt: Appel</div>
          <div className="rounded-lg p-3 text-center font-black text-lg" style={{
            backgroundColor: tp > 0 ? 'rgba(95, 148, 125, 0.08)' : '#e3e2dc',
            color: tp > 0 ? '#202023' : '#6f6e69'
          }}>
            {tp}
            <p className="text-[8px] font-bold uppercase" style={{ color: 'rgba(95, 148, 125, 0.7)' }}>Correct</p>
          </div>
          <div className="rounded-lg p-3 text-center font-black text-lg" style={{
            backgroundColor: fn > 0 ? 'rgba(217, 120, 72, 0.08)' : '#e3e2dc',
            color: fn > 0 ? '#ff3c21' : '#6f6e69'
          }}>
            {fn}
          ...
          {/* Row 2: Echt Banaan */}
          <div className="text-right text-[10px] font-bold py-3 pr-2" style={{ color: '#e1ff01' }}>Echt: Banaan</div>
          <div className="rounded-lg p-3 text-center font-black text-lg" style={{
            backgroundColor: fp > 0 ? 'rgba(217, 120, 72, 0.08)' : '#e3e2dc',
            color: fp > 0 ? '#ff3c21' : '#6f6e69'
          }}>
            {fp}
            <p className="text-[8px] font-bold uppercase" style={{ color: 'rgba(217, 120, 72, 0.7)' }}>Fout</p>
          </div>
          <div className="rounded-lg p-3 text-center font-black text-lg" style={{
            backgroundColor: tn > 0 ? 'rgba(95, 148, 125, 0.08)' : '#e3e2dc',
            color: tn > 0 ? '#202023' : '#6f6e69'
          }}>
            {tn}
            <p className="text-[8px] font-bold uppercase" style={{ color: 'rgba(95, 148, 125, 0.7)' }}>Correct</p>
          </div>
      ...
      <div className="rounded-2xl p-4 text-center" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
        <p className="text-[10px] uppercase tracking-wider font-bold mb-2" style={{ color: '#6f6e69' }}>Nauwkeurigheid</p>
        <p className="text-4xl font-black" style={{ color: accuracy >= 80 ? '#202023' : accuracy >= 50 ? '#ff3c21' : '#ff3c21' }}>
          {accuracy.toFixed(0)}%
        </p>
        <div className="w-full rounded-full h-3 mt-3 overflow-hidden" style={{ backgroundColor: '#e3e2dc' }}>
          <div
...
export const MLTrainerMission: React.FC<Props> = ({ onBack, onComplete }) => {
  ...
  if (phase === 'intro') {
    return (
      <div className="min-h-screen text-white overflow-y-auto p-4 pb-safe" style={{ backgroundColor: '#f2f1ec' }}>
        <button onClick={onBack} className="flex items-center gap-2 mb-6 transition-colors" style={{ color: '#6f6e69' }}
        ...
        <div className="max-w-lg mx-auto text-center space-y-6">
          ...
          <h1 className="text-3xl font-black" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#202023' }}>ML Trainer</h1>
          <p className="text-sm leading-relaxed max-w-sm mx-auto" style={{ fontFamily: "'Outfit', system-ui, sans-serif", color: '#6f6e69' }}>
            Leer hoe <span className="font-bold" style={{ color: '#202023' }}>Machine Learning</span> werkt door zelf een model te trainen!
          ...
          <div className="rounded-2xl p-5 space-y-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
            <h3 className="text-sm font-bold" style={{ color: '#202023' }}>Hoe traint een ML-model?</h3>
            <div className="grid grid-cols-1 gap-3">
              <div className="flex items-start gap-3 text-left">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ backgroundColor: '#ff3c21' }}>1</div>
                <div>
                  <p className="text-sm font-bold" style={{ color: '#202023' }}>Data labelen</p>
                  <p className="text-xs" style={{ color: '#6f6e69' }}>Vertel het model wat elk voorbeeld is (bijv. appel of banaan)</p>
                </div>
              </div>
              <div className="flex items-start gap-3 text-left">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ backgroundColor: '#202023' }}>2</div>
                <div>
                  <p className="text-sm font-bold" style={{ color: '#202023' }}>Train/Test splitsen</p>
                  <p className="text-xs" style={{ color: '#6f6e69' }}>Verdeel de data: een deel om van te leren, een deel om te testen</p>
                </div>
              </div>
              <div className="flex items-start gap-3 text-left">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ backgroundColor: '#202023' }}>3</div>
                <div>
                  <p className="text-sm font-bold" style={{ color: '#202023' }}>Evalueren</p>
                  <p className="text-xs" style={{ color: '#6f6e69' }}>Kijk hoe goed het model presteert op de testdata</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Example */}
          <div className="rounded-2xl p-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
            <p className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: '#6f6e69' }}>Voorbeeld: Fruit Classificatie</p>
            <div className="flex items-center justify-center gap-4">
              <div className="rounded-xl p-3 text-center" style={{ backgroundColor: 'rgba(217, 120, 72, 0.06)', border: '1px solid rgba(217, 120, 72, 0.2)' }}>
                ...
                <p className="text-[10px] font-bold mt-1" style={{ color: '#ff3c21' }}>rood, rond, klein</p>
              </div>
              <span className="text-xl" style={{ color: '#e3e2dc' }}>vs</span>
              <div className="rounded-xl p-3 text-center" style={{ backgroundColor: 'rgba(215, 201, 95, 0.06)', border: '1px solid rgba(215, 201, 95, 0.2)' }}>
                <span className="text-2xl">🍌</span>
                <p className="text-[10px] font-bold mt-1" style={{ color: '#e1ff01' }}>geel, lang, krom</p>
              </div>
            </div>
          </div>
  ...
  if (phase === 'labeling') {
    ...
    return (
      <div className="min-h-screen overflow-y-auto pb-safe" style={{ backgroundColor: '#f2f1ec' }}>
        {/* Header */}
        <div className="sticky top-0 z-20 backdrop-blur-md" style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderBottom: '1px solid #E7D8BD' }}>
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            <button onClick={onBack} className="p-2 transition-colors" style={{ color: '#6f6e69' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#202023')}
              onMouseLeave={e => (e.currentTarget.style.color = '#6f6e69')}
            ><ArrowLeft size={20} /></button>
            <div className="flex items-center gap-2">
              <Tag size={16} style={{ color: '#ff3c21' }} />
              <span className="text-sm font-bold" style={{ color: '#202023' }}>{labeledCount}/{items.length} gelabeld</span>
            </div>
            {allLabeled && (
              <button
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          <div className="text-center space-y-1">
            <h2 className="text-xl font-black" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#202023' }}>Stap 1: Data Labelen</h2>
            <p className="text-sm" style={{ color: '#6f6e69' }}>
              Bekijk de kenmerken en beslis: is het een <span className="font-bold" style={{ color: '#ff3c21' }}>appel</span> of een <span className="font-bold" style={{ color: '#e1ff01' }}>banaan</span>?
            </p>
          </div>
          ...
          {currentItem && (
            <div className="rounded-2xl p-5 space-y-4" style={{ backgroundColor: '#FFFFFF', border: '2px solid rgba(217, 120, 72, 0.3)' }}>
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-wider font-bold mb-2" style={{ color: '#ff3c21' }}>Item #{currentItem.id}</p>
                <div className="flex flex-wrap gap-2 justify-center">
                  {currentItem.features.map((f, i) => (
                    <span key={i} className="text-sm px-3 py-1.5 rounded-full font-bold" style={{ backgroundColor: '#e3e2dc', color: '#202023', border: '1px solid #E7D8BD' }}>
                      {f}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  style={{
                    backgroundColor: currentItem.label === 'appel' ? '#ff3c21' : 'rgba(217, 120, 72, 0.06)',
                    color: currentItem.label === 'appel' ? '#FFFFFF' : '#ff3c21',
                    border: `2px solid ${currentItem.label === 'appel' ? '#ff3c21' : 'rgba(217, 120, 72, 0.2)'}`,
                ...
                <button
                  ...
                  className="flex-1 py-4 rounded-full font-black text-lg flex items-center justify-center gap-2 transition-all duration-300 active:scale-95"
                  style={{
                    backgroundColor: currentItem.label === 'banaan' ? '#e1ff01' : 'rgba(215, 201, 95, 0.06)',
                    color: currentItem.label === 'banaan' ? '#FFFFFF' : '#e1ff01',
                    border: `2px solid ${currentItem.label === 'banaan' ? '#e1ff01' : 'rgba(215, 201, 95, 0.2)'}`,
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color: '#6f6e69' }}>Alle items (klik om te selecteren)</p>
  ...
  if (phase === 'splitting') {
    return (
      <div className="min-h-screen overflow-y-auto pb-safe" style={{ backgroundColor: '#f2f1ec' }}>
        ...
        <div className="sticky top-0 z-20 backdrop-blur-md" style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderBottom: '1px solid #E7D8BD' }}>
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            <button onClick={() => setPhase('labeling')} className="p-2 transition-colors" style={{ color: '#6f6e69' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#202023')}
              onMouseLeave={e => (e.currentTarget.style.color = '#6f6e69')}
            ><ArrowLeft size={20} /></button>
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold px-2 py-1 rounded-full" style={{ backgroundColor: 'rgba(217, 120, 72, 0.1)', color: '#ff3c21', border: '1px solid rgba(217, 120, 72, 0.2)' }}>Train: {trainItems.length}</span>
              <span className="text-xs font-bold px-2 py-1 rounded-full" style={{ backgroundColor: 'rgba(11, 69, 63, 0.1)', color: '#202023', border: '1px solid rgba(11, 69, 63, 0.2)' }}>Test: {testItems.length}</span>
            </div>
            {canTrain && (
              <button
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          <div className="text-center space-y-1">
            <h2 className="text-xl font-black" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#202023' }}>Stap 2: Train/Test Split</h2>
            <p className="text-sm" style={{ color: '#6f6e69' }}>
              Klik op items om ze toe te wijzen: <span className="font-bold" style={{ color: '#ff3c21' }}>Train</span> (om van te leren)
              of <span className="font-bold" style={{ color: '#202023' }}>Test</span> (om te testen). Minimaal 2 in elk!
            </p>
          </div>
          ...
          <div className="rounded-2xl p-3" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
            <div className="flex items-start gap-2">
              <Sparkles size={14} className="mt-0.5 flex-shrink-0" style={{ color: '#ff3c21' }} />
              <p className="text-[11px]" style={{ color: '#6f6e69' }}>
                <span className="font-bold" style={{ color: '#ff3c21' }}>Waarom splitsen?</span> Als je het model test met dezelfde data als waarmee het getraind is,
          ...
          <div className="flex items-center justify-center gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded" style={{ backgroundColor: '#e3e2dc' }} />
              <span style={{ color: '#6f6e69' }}>Niet toegewezen (klik)</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded" style={{ backgroundColor: '#ff3c21' }} />
              <span style={{ color: '#ff3c21' }}>Train (klik nogmaals)</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded" style={{ backgroundColor: '#202023' }} />
              <span style={{ color: '#202023' }}>Test (klik nog eens = reset)</span>
            </div>
          </div>
          ...
          {!canTrain && (trainItems.length > 0 || testItems.length > 0) && (
            <div className="rounded-2xl p-3 text-center" style={{ backgroundColor: 'rgba(217, 120, 72, 0.06)', border: '1px solid rgba(217, 120, 72, 0.2)' }}>
              <p className="text-sm font-bold" style={{ color: '#ff3c21' }}>
    ...
    <div className="min-h-screen overflow-y-auto" style={{ backgroundColor: '#f2f1ec' }}>
      <div className="min-h-full flex items-center justify-center p-4 pb-safe">
        <div className="max-w-md w-full space-y-6">
          <div className="text-center">
            <h1 className="text-2xl font-black mb-2" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#202023' }}>Model Getraind!</h1>
            <p className="text-sm" style={{ color: '#6f6e69' }}>Zo presteert jouw ML-model op de testdata:</p>
          </div>
          ...
          <div className="rounded-2xl p-4 space-y-2" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
            <h3 className="text-sm font-bold flex items-center gap-2" style={{ color: '#ff3c21' }}>
              <Sparkles size={14} /> Wat betekent dit?
            </h3>
            <p className="text-xs" style={{ color: '#6f6e69' }}>
            ...
            <p className="text-xs" style={{ color: '#6f6e69' }}>
              <span className="font-bold" style={{ color: '#ff3c21' }}>Fun fact:</span> echte ML-engineers doen precies dit proces, maar dan
          ...
          >
            <div className="rounded-xl p-4 mt-2" style={{ backgroundColor: '#f2f1ec' }}>
              <p className="text-sm font-medium mb-1" style={{ color: '#6f6e69' }}>Detailscore</p>
              <div className="flex justify-between text-sm">
                <span style={{ color: '#6f6e69' }}>Labels correct: {correctLabels}/{items.length}</span>
                <span style={{ color: '#6f6e69' }}>Model nauwkeurigheid: {accuracy.toFixed(0)}%</span>
              </div>
            </div>
          </MissionFeedbackCard>

          <div className="rounded-2xl p-4 text-left space-y-2" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E7D8BD' }}>
            <p className="text-xs font-bold mb-2" style={{ color: '#202023' }}>Wat je hebt geleerd:</p>
            <p className="text-xs" style={{ color: '#6f6e69' }}>1. ML-modellen leren van gelabelde data (supervised learning)</p>
            <p className="text-xs" style={{ color: '#6f6e69' }}>2. Train/test split is nodig om eerlijk te meten hoe goed een model is</p>
            <p className="text-xs" style={{ color: '#6f6e69' }}>3. Een confusion matrix laat zien waar het model fouten maakt</p>
          </div>
          ...
          <div className="rounded-2xl p-4 text-left space-y-3" style={{ backgroundColor: '#FFFFFF', border: '1px solid rgba(217, 120, 72, 0.2)' }}>
            ...
            <textarea
              ...
              style={{ minHeight: '80px', fontFamily: "'Outfit', system-ui, sans-serif", borderColor: '#e3e2dc', backgroundColor: '#f2f1ec', fontSize: '0.875rem' }}
              onFocus={e => (e.currentTarget.style.borderColor = '#ff3c21')}
              onBlur={e => (e.currentTarget.style.borderColor = '#e3e2dc')}
            />
          </div>
          ...
          <button
            ...
            onMouseEnter={e => { if (saved.reflectie.trim().length >= 10) e.currentTarget.style.backgroundColor = '#ff3c21'; }}
            onMouseLeave={e => { if (saved.reflectie.trim().length >= 10) e.currentTarget.style.backgroundColor = '#ff3c21'; }}
          >
