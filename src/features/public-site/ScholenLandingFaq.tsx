...
const FAQ_ITEMS: { category?: string; q: string; a: string }[] = [
    ...
    { category: 'Technisch & privacy', q: 'Hoe ondersteunt DGSkills AVG en privacy?', a: 'DGSkills is privacy-by-design ingericht met een DPA-template, DPIA-ondersteuning en beveiligingsmaatregelen. Definitieve inzet vereist school-DPIA, subprocessorcheck en FG/DPO-review.' },
    ...
    { category: 'Technisch & privacy', q: 'Hoe gaat DGSkills om met de EU AI Act?', a: 'DGSkills behandelt school-facing AI als hoog-risico onderwijs-AI onder EU AI Act Annex III punt 3(b) waar AI leerresultaten evalueert of het leerproces stuurt. Art. 4 AI-geletterdheid geldt sinds 2 februari 2025; Art. 50-transparantie vanaf augustus 2026; volgens actuele Commissie-informatie gelden high-risk onderwijsverplichtingen vanaf 2 december 2027. De Compliance Hub beschrijft de voorbereiding; definitieve schoolinzet vereist DPA, school-DPIA, subprocessorcheck en FG/DPO-review.' },
    ...
    return (
        <div>
            <div className="max-w-3xl mx-auto">
                <h2 className="text-2xl md:text-3xl font-medium tracking-tight mb-3" style={{ fontFamily: "'Newsreader', Georgia, serif", color: '#202023' }}>
                    Veelgestelde vragen
                </h2>
                <p className="text-base leading-relaxed mb-10" style={{ color: '#202023' }}>
                ...
                <div className="space-y-8">
                    {allowedCategories.map(cat => {
                        ...
                        return (
                            <div key={cat}>
                                <h3 className="text-sm font-medium uppercase tracking-wide mb-4" style={{ color: '#ff3c21' }}>{cat}</h3>
                                <div className="space-y-3">
                                    {items.map((item) => {
                                        ...
                                        return (
                                            <div key={i} className="bg-white rounded-xl overflow-hidden shadow-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#e3e2dc' }}>
                                                <button
                                                    onClick={() => setOpenFaq(isOpen ? null : i)}
                                                    className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-duck-bg/50 transition-colors focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-duck-coral focus-visible:rounded-xl"
                                                    aria-expanded={isOpen}
                                                    aria-controls={`faq-answer-${i}`}
                                                >
                                                    <span className="text-[15px] font-medium pr-4 leading-snug" style={{ color: '#202023' }}>{item.q}</span>
                                                    <IconChevronDown className={`flex-shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} style={{ color: '#202023' }} />
                                                </button>
                                                {isOpen && (
                                                    <div id={`faq-answer-${i}`} className="px-5 pb-5 pt-0">
                                                        <p className="text-[15px] leading-relaxed" style={{ color: '#202023' }}>{item.a}</p>
                                                        {showCta && (
                                                            <button
                                                                onClick={scrollToContact}
                                                                className="mt-4 text-sm font-semibold flex items-center gap-1.5 transition-colors"
                                                                style={{ color: '#ff3c21' }}
                                                                onMouseEnter={(e) => (e.currentTarget.style.color = '#D97848')}
                                                                onMouseLeave={(e) => (e.currentTarget.style.color = '#D97848')}
                                                            >
                                                                Pilot aanvragen
                                                                <IconArrowRight />
                                                            </button>
                ...
                <div className="mt-12 pt-8 text-center" style={{ borderTopWidth: '1px', borderTopStyle: 'solid', borderTopColor: '#E7D8BD' }}>
                    <p className="text-sm mb-4" style={{ color: '#202023' }}>Nog vragen? We staan voor je klaar.</p>
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <button
                            onClick={scrollToContact}
                            className="text-sm font-semibold text-white px-5 py-2.5 rounded-full inline-flex items-center gap-1.5 transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-duck-coral"
                        ...
                        <a
                            ...
                            style={{ color: '#202023', borderWidth: '1px', borderStyle: 'solid', borderColor: '#e3e2dc' }}
                            onMouseEnter={(e) => { e.currentTarget.style.color = '#D97848'; e.currentTarget.style.borderColor = '#D97848'; }}
                            onMouseLeave={(e) => { e.currentTarget.style.color = '#445865'; e.currentTarget.style.borderColor = '#E7D8BD'; }}
                        >
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
