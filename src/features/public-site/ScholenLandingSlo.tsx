...
const domains: SloDomain[] = [
    {
        ...
        accent: 'bg-duck-coral',
        accentBg: 'bg-duck-bg',
        accentLight: 'bg-duck-line',
        color: 'text-duck-coral',
            ...
            <div className="max-w-5xl mx-auto">
                <div className="md:flex md:items-start md:justify-between md:gap-10 mb-14">
                    <div className="max-w-lg mb-8 md:mb-0">
                        ...
                        <p className="text-base leading-relaxed" style={{ color: '#202023' }}>
                    ...
                    <div className="hidden md:flex items-center justify-center flex-shrink-0">
                        <img src="/assets/brand/dgskills-duck-guide-v3.png" alt="" className="w-16 h-16 object-contain" loading="lazy" decoding="async" aria-hidden="true" />
                    </div>
                    <div className="flex flex-col items-end gap-3">
                        <div className="flex flex-wrap gap-2">
                            {['MAVO', 'HAVO', 'VWO'].map(level => (
                                <span key={level} className="text-xs font-medium bg-white px-3 py-1.5 rounded-md" style={{ color: '#202023', borderWidth: '1px', borderStyle: 'solid', borderColor: '#e3e2dc' }}>{level}</span>
                            ))}
                        </div>
                        {/* Progress indicator */}
                        <div className="bg-white rounded-xl px-5 py-3 flex items-center gap-4 shadow-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#e3e2dc' }}>
                            <div className="relative w-12 h-12" role="img" aria-label="100% van alle kerndoelen gedekt">
                                <svg className="w-12 h-12 -rotate-90" viewBox="0 0 48 48">
                                    <circle cx="24" cy="24" r="20" fill="none" stroke="#E7D8BD" strokeWidth="4" />
                                    <circle cx="24" cy="24" r="20" fill="none" stroke="#D97848" strokeWidth="4"
                                        strokeDasharray={`${2 * Math.PI * 20}`}
                                        strokeDashoffset="0"
                                        strokeLinecap="round"
                                    />
                                </svg>
                                <span className="absolute inset-0 flex items-center justify-center text-xs font-black" style={{ color: '#ff3c21' }}>100%</span>
                            </div>
                            <div>
                                <p className="text-sm font-bold" style={{ color: '#202023' }}>{totalKerndoelen} van {totalKerndoelen}</p>
                                <p className="text-[11px]" style={{ color: '#202023' }}>Kerndoelen gedekt</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        return (
                            ...
                            >
                                <button
                                    ...
                                    className="w-full p-6 flex items-start gap-4 text-left focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-duck-coral focus-visible:rounded-2xl"
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-base font-medium mb-1" style={{ color: '#202023' }}>{domain.name}</p>
                                        <p className="text-sm" style={{ color: '#202023' }}>{domain.description}</p>
                                        <p className="text-xs mt-2 font-medium" style={{ color: '#202023' }}>{domain.kerndoelen.length} kerndoelen</p>
                                    </div>
                                    <IconChevronDown className={`flex-shrink-0 mt-1 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} style={{ color: '#202023' }} />
                                </button>

                                {isExpanded && (
                                    <div id={`domain-panel-${idx}`} className="px-6 pb-6 pt-0">
                                        <div className="pt-4 space-y-2.5" style={{ borderTopWidth: '1px', borderTopStyle: 'solid', borderTopColor: '#E7D8BD' }}>
                                            {domain.kerndoelen.map(kd => (
                                                <div key={kd.id} className="flex items-start gap-3 bg-white/80 rounded-xl px-4 py-3" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#e3e2dc' }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium leading-snug" style={{ color: '#202023' }}>{kd.title}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                ...
                <p className="text-xs mt-6 text-center" style={{ color: '#202023' }}>De kerndoelen zijn gebaseerd op de SLO-conceptkerndoelen (september 2025). Definitieve vaststelling volgt via een AMvB.</p>
            </div>
        </div>
    );
};
