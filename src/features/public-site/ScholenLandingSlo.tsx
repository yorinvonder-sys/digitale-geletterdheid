            ...
            <div className="max-w-5xl mx-auto">
                <div className="md:flex md:items-start md:justify-between md:gap-10 mb-14">
                    <div className="max-w-lg mb-8 md:mb-0">
                        ...
                        <p className="text-base leading-relaxed" style={{ color: '#445865' }}>
                    ...
                    <div className="hidden md:flex items-center justify-center flex-shrink-0">
                        <img src="/assets/storytelling/beaver-storyteller.webp" alt="" className="w-16 h-16 object-contain" loading="lazy" decoding="async" aria-hidden="true" />
                    </div>
                    <div className="flex flex-col items-end gap-3">
                        <div className="flex flex-wrap gap-2">
                            {['MAVO', 'HAVO', 'VWO'].map(level => (
                                <span key={level} className="text-xs font-medium bg-white px-3 py-1.5 rounded-md" style={{ color: '#445865', borderWidth: '1px', borderStyle: 'solid', borderColor: '#E7D8BD' }}>{level}</span>
                            ))}
                        </div>
                        {/* Progress indicator */}
                        <div className="bg-white rounded-xl px-5 py-3 flex items-center gap-4 shadow-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#E7D8BD' }}>
                            <div className="relative w-12 h-12" role="img" aria-label="100% van alle kerndoelen gedekt">
                                ...
                                <span className="absolute inset-0 flex items-center justify-center text-xs font-black" style={{ color: '#D97848' }}>100%</span>
                            </div>
                            <div>
                                <p className="text-sm font-bold" style={{ color: '#08283B' }}>{totalKerndoelen} van {totalKerndoelen}</p>
                                <p className="text-[11px]" style={{ color: '#445865' }}>Kerndoelen gedekt</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        return (
                            ...
                            >
                                <button
                                    ...
                                    className="w-full p-6 flex items-start gap-4 text-left focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#D97848] focus-visible:rounded-2xl"
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-base font-medium mb-1" style={{ color: '#08283B' }}>{domain.name}</p>
                                        <p className="text-sm" style={{ color: '#445865' }}>{domain.description}</p>
                                        <p className="text-xs mt-2 font-medium" style={{ color: '#445865' }}>{domain.kerndoelen.length} kerndoelen</p>
                                    </div>
                                    <IconChevronDown className={`flex-shrink-0 mt-1 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} style={{ color: '#445865' }} />
                                </button>
                                    ...
                                    <div id={`domain-panel-${idx}`} className="px-6 pb-6 pt-0">
                                        <div className="pt-4 space-y-2.5" style={{ borderTopWidth: '1px', borderTopStyle: 'solid', borderTopColor: '#E7D8BD' }}>
                                            {domain.kerndoelen.map(kd => (
                                                <div key={kd.id} className="flex items-start gap-3 bg-white/80 rounded-xl px-4 py-3" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#E7D8BD' }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium leading-snug" style={{ color: '#08283B' }}>{kd.title}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                ...
                <p className="text-xs mt-6 text-center" style={{ color: '#445865' }}>De kerndoelen zijn gebaseerd op de SLO-conceptkerndoelen (september 2025). Definitieve vaststelling volgt via een AMvB.</p>
            </div>
        </div>
    );
};
