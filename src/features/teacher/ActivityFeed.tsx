    ...
    return (
        <div className="bg-white rounded-3xl border border-lab-line shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-4 border-b border-lab-line flex items-center justify-between">
                <h3 className="text-xs font-black text-lab-ink uppercase tracking-widest flex items-center gap-2">
                    <div className="w-2 h-2 bg-lab-coral rounded-full animate-pulse"></div>
                    Live Activiteit
                </h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {activities.length === 0 ? (
                    ...
                    <div key={activity.id} className="flex gap-3 group">
                        <div className="mt-1 w-8 h-8 rounded-xl bg-lab-cream border border-lab-line flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            {getIcon(activity.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                                <span className="text-sm font-bold text-lab-ink truncate">{activity.studentName}</span>
