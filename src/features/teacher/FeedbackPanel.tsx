...
export const FeedbackPanel: React.FC<FeedbackPanelProps> = ({ schoolId }) => {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div className="bg-duck-ink rounded-[1.75rem] p-6 md:p-8 text-white shadow-xl">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {filteredFeedback.length > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 text-duck-ink/60 text-sm font-bold hover:text-duck-ink transition-colors"
                        >
                            {selectedIds.size === filteredFeedback.length ? <CheckSquare size={18} className="text-duck-acid" /> : <Square size={18} />}
                            Selecteer alles
                        </button>
                    )}
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    {selectedIds.size > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-duck-error text-white hover:bg-duck-ink hover:text-white rounded-full font-bold text-sm transition-colors animate-in fade-in"
                        >
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-dashed border-duck-ink/15 p-12 text-center">
                    <MessageSquare size={48} className="text-duck-ink/40 mx-auto mb-4" />
                ...
                <div className="space-y-3">
                    {filteredFeedback.map((item) => (
                        <div
                            key={item.id}
                            className={`bg-white rounded-2xl border p-5 shadow-sm hover:shadow-md transition-shadow relative ${selectedIds.has(item.id!) ? 'border-duck-ink ring-1 ring-duck-ink/20 bg-duck-bgLight' : 'border-duck-ink/15'}`}
                        >
                            <div className="flex items-start gap-4">
                                <button
                                    onClick={() => item.id && toggleSelect(item.id)}
                                    className="mt-1 text-duck-ink/60 hover:text-duck-ink transition-colors shrink-0"
                                >
                                    {selectedIds.has(item.id!) ? <CheckSquare size={20} className="text-duck-ink" /> : <Square size={20} />}
                                </button>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs text-duck-ink/60 flex items-center gap-2">
                                                <span className="bg-duck-bgLight px-2 py-0.5 rounded-full">{item.user_class}</span>
                                    ...
                                    <p className="text-duck-ink/60 text-sm leading-relaxed bg-duck-bgLight p-4 rounded-xl">
                                        {item.message}
                                    </p>
                                </div>
                                ...
                                <button
                                    ...
                                    className="p-2 text-duck-ink/60 hover:bg-duck-error hover:text-white rounded-lg transition-colors shrink-0"
                                    title="Verwijderen"
                                >
                                    {deleting === item.id ? (
