    ...
    return (
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                    {/* Header */}
                    <div className="p-6 border-b border-duck-ink/15 bg-duck-ink">
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 bg-duck-acid rounded-2xl flex items-center justify-center text-duck-ink shadow-lg">
                                    <Target size={28} />
                                </div>
                                <div>
                                    <h2 className="text-xl font-black text-white">Focus Modus Activeren</h2>
                            ...
                            <div className="flex gap-3 mt-4">
                                ...
                                {selectedClass && (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 bg-duck-acid text-duck-ink rounded-xl text-sm font-bold hover:bg-duck-acid transition-colors"
                                    >
                                        <Users size={14} />
                                        {selectedClass}
                                        <X size={14} />
                                ...
                                <div className="flex-1 relative">
                                    <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-duck-ink/40" />
                                    <input
                                        type="text"
                                        value={searchTerm}
                                        onChange={(e) => setSearchTerm(e.target.value)}
                                        placeholder="Zoek opdracht..."
                                        className="w-full pl-11 pr-4 py-2.5 bg-white border border-duck-ink/15 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-duck-ink/10 focus:border-duck-ink"
                                    />
                                </div>
                                <div className="flex bg-white/10 p-1 rounded-xl">
                                    <button
                                        onClick={() => setSelectedWeek(null)}
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedWeek === null ? 'bg-duck-acid text-duck-ink shadow-sm' : 'text-white/60'}`}
                                    >
                                        Alle
                                    </button>
                                    {availableWeeks.map(week => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedWeek === week ? 'bg-duck-acid text-duck-ink shadow-sm' : 'text-white/60'}`}
                                        >
                                            W{week}
                                        </button>
                                    ))}
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                                    ...
                                    {availableClasses.map(className => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className="group p-5 bg-duck-bgLight hover:bg-duck-acid border border-duck-ink/15 hover:border-duck-acid rounded-xl text-left transition-all hover:shadow-md"
                                            >
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-12 h-12 bg-white group-hover:bg-duck-ink rounded-xl flex items-center justify-center text-duck-ink/60 group-hover:text-duck-acid transition-colors border border-duck-ink/15 font-black text-lg">
                                                            {className}
                                                        </div>
                                                        <div>
                                                            ...
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <div className="flex items-center gap-1">
                                                                    <div className={`w-2 h-2 rounded-full ${activeCount > 0 ? 'bg-duck-acid animate-pulse' : 'bg-duck-gray'}`} />
                                                                    <span className={`text-xs font-bold ${activeCount > 0 ? 'text-duck-ink' : 'text-duck-ink/60'}`}>
                                                                        {activeCount} online
                                                                    </span>
                                                                </div>
                                                                <span className="text-duck-ink/40">•</span>
                                                                <span className="text-xs text-duck-ink/60">{totalCount} totaal</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <ChevronRight size={20} className="text-duck-ink/40 group-hover:text-duck-ink transition-colors" />
                                                </div>
                                            </button>
                                        );
                                    })}
                                ...
                                >
                                    {filteredMissions.map(mission => (
                                        <button
                                            ...
                                            className="group p-4 bg-duck-bgLight hover:bg-duck-acid border border-duck-ink/15 hover:border-duck-acid rounded-xl text-left transition-all hover:shadow-md"
                                        >
                                            <div className="flex items-start gap-3">
                                                <div className="w-10 h-10 bg-white group-hover:bg-duck-ink rounded-xl flex items-center justify-center text-duck-ink/60 group-hover:text-duck-acid transition-colors border border-duck-ink/15">
                                                    <Target size={20} />
                                                </div>
                        ...
                        {!showClassSelector && filteredMissions.length === 0 && (
                            <div className="text-center py-12">
                                <Search size={40} className="text-duck-ink/40 mx-auto mb-3" />
                    ...
                    {/* Footer */}
                    <div className="p-4 border-t border-duck-ink/15 bg-duck-bgLight">
