    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {highlights.length === 0 ? (
                    <div className="col-span-full py-20 text-center bg-duck-bg rounded-[2rem] border-2 border-dashed border-duck-ink/15">
                        <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 text-duck-ink/60">
                            <Star size={32} />
                        </div>
                        <p className="text-duck-ink/60 font-bold uppercase tracking-widest text-xs">Nog geen werk gehighlight</p>
                    </div>
                ) : highlights.map((work) => (
                    <div key={work.id} className="bg-white rounded-[2rem] p-8 shadow-xl shadow-duck-ink/15/50 border border-duck-ink/15 relative group overflow-hidden transition-all hover:shadow-2xl">
                        ...
                        <div className="flex items-center gap-3 mb-6">
                            <div className="w-12 h-12 bg-duck-error text-white rounded-2xl flex items-center justify-center font-black">
                                {work.studentName[0]}
                            </div>
                            <div>
                        ...
                        <div className="bg-duck-bg rounded-2xl p-6 relative mb-6">
                            <Quote className="absolute -top-3 -left-2 text-duck-error" size={32} />
                        ...
                        {work.teacherNote && (
                            <div className="flex items-start gap-2 text-[11px] font-bold text-duck-ink/60 bg-duck-bg/50 p-3 rounded-xl border border-dashed border-duck-ink/15">
                                <Code size={14} className="mt-0.5 text-duck-error" />
