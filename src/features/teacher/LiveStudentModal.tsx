...
export const LiveStudentModal: React.FC<LiveStudentModalProps> = ({ student, onClose }) => {
    ...
    const getActivityColor = (type: string) => {
        switch (type) {
            case 'login': return 'text-duck-acid';
            case 'mission_complete': return 'text-duck-acid';
            case 'badge_earned': return 'text-duck-acid';
            case 'focus_lost': return 'text-duck-error';
            case 'xp_earned': return 'text-duck-acid';
            case 'test_taken': return 'text-duck-acid';
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 font-mono" onClick={onClose}>
            <div role="dialog" aria-modal="true" aria-label="Live meekijken terminal" className="bg-duck-ink rounded-lg shadow-2xl border border-duck-ink/15 w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                ...
                {/* TERMINAL HEADER */}
                <div className="bg-duck-ink px-4 py-2 border-b border-white/10 flex items-center justify-between select-none">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5">
                            <span className="relative flex h-2 w-2">
                                <span className="animate-ping motion-reduce:animate-none absolute inline-flex h-full w-full rounded-full bg-duck-acid opacity-75"></span>
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-duck-acid"></span>
                            </span>
                            <span className="text-[10px] text-white/40 font-bold tracking-widest uppercase">LIVE</span>
                        </div>
                        <button onClick={onClose} className="hover:bg-white/10 p-1 rounded transition-colors text-white/40 hover:text-white min-h-[44px] min-w-[44px] flex items-center justify-center" aria-label="Sluiten">
                            <X size={16} />
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-xs md:text-sm bg-duck-ink text-white/75 relative" ref={scrollRef}>
                    ...
                    <div className="space-y-1 mb-4 border-b border-white/10 pb-4">
                        <div className="text-white/40">Beveiligde protocollen initialiseren...</div>
                        {connectionStep >= 1 && <div className="text-white/40">Authenticeren: <span className="text-duck-acid">OK</span></div>}
                        {connectionStep >= 2 && <div className="text-white/40">Real-time verbinding maken... <span className="text-duck-acid">VERBONDEN</span></div>}
                        {connectionStep >= 3 && <div className="text-white/40">Studentactiviteit decoderen...</div>}
                    </div>

                    {/* ACTIVITY FEED */}
                    {connectionStep >= 3 && (
                        loading ? (
                            <div className="p-4 flex items-center justify-center gap-2 text-duck-acid animate-pulse motion-reduce:animate-none">
                            ...
                            <div className="space-y-3">
                                {activities.map((activity) => (
                                    <div key={activity.id} className="flex gap-3 group hover:bg-white/5 p-1 rounded transition-colors">
                                        <div className="min-w-[80px] text-white/40 text-[10px] pt-0.5">
                                            {activity.timestamp ?
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {activity.type === 'mission_start' && <div className="text-white/40 pl-2 border-l border-white/10 ml-1 mt-1 text-[10px]">Omgeving initialiseren...</div>}
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    {connectionStep >= 3 && (
                        <div className="mt-4 flex items-center gap-2 text-white/40 animate-pulse motion-reduce:animate-none">
                ...
                {/* TERMINAL FOOTER */}
                <div className="bg-duck-ink p-2 border-t border-white/10 flex justify-between items-center text-[10px] text-white/70 font-mono">
