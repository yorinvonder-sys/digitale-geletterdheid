                    ...
                    <div className="p-6 space-y-5">
                        {!result ? (
                            <>
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-4 py-3 pr-12 bg-duck-bg border-2 border-duck-ink/15 rounded-xl text-sm font-medium text-duck-ink placeholder:text-duck-ink/40
                                                     focus:ring-4 focus:ring-duck-ink/10 focus:border-duck-ink outline-none transition-all"
                                        />
                                        <button
                                            type="button"
                                            onClick={() => setShowPassword(!showPassword)}
                                            className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-duck-ink/60 hover:text-duck-ink transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                                ...
                                <div className="bg-duck-bgLight border border-duck-gray rounded-xl p-4">
                            ...
                            <div className={`rounded-xl p-5 ${result.success ? 'bg-duck-acid/20 border border-duck-ink/15' : 'bg-duck-error/10 border border-duck-error'}`}>
                                <div className="flex items-start gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${result.success ? 'bg-duck-acid text-duck-ink' : 'bg-duck-error text-white'}`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className={`text-xs mt-0.5 ${result.success ? 'text-duck-ink/60' : 'text-duck-error'}`}>
                                            {result.message}
                                        </p>
                    ...
                    <div className="px-6 py-4 bg-duck-bgLight border-t border-duck-ink/15 flex justify-end gap-3">
                        ...
                        {!result && (
                            <button
                                ...
                                className="px-5 py-2.5 bg-duck-error text-white font-bold text-sm rounded-full
                                         hover:bg-duck-ink hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 shadow-lg"
                            >
                                {isResetting ? (
                                    <>
