    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                className="fixed inset-0 z-[60] bg-duck-ink/40 backdrop-blur-sm flex items-center justify-center p-4"
                onClick={onClose}
            >
                <motion.div
                    ...
                    className="bg-white rounded-[1.75rem] shadow-duck-soft border border-duck-ink/15 max-w-lg w-full overflow-hidden max-h-[90vh] flex flex-col"
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Header */}
                    <div className="bg-duck-ink px-6 py-5 text-white">
                    ...
                    <div className="p-6 space-y-4 overflow-y-auto">
                        {!result ? (
                            <>
                                ...
                                {rows.length > 0 && (
                                    <div className="bg-duck-acid/20 border border-duck-ink/15 rounded-xl p-3 text-sm text-duck-ink flex items-center gap-2">
                                ...
                                {parseErrors.length > 0 && (
                                    <div className="bg-duck-bgLight border border-duck-gray rounded-xl p-3 max-h-32 overflow-y-auto">
                                ...
                                {error && (
                                    <div className="bg-duck-error/10 border border-duck-error rounded-xl p-3 text-sm text-duck-error flex items-start gap-2">
                            ...
                            <>
                                <div className="bg-duck-acid/20 border border-duck-ink/15 rounded-xl p-4">
                                ...
                                {result.credentials.length > 0 && (
                                    <div className="bg-duck-error/10 border border-duck-error rounded-xl p-4 space-y-2">
                                        ...
                                        <button onClick={downloadCredentials} className="w-full px-4 py-2.5 bg-duck-acid text-duck-ink font-bold text-sm rounded-full flex items-center justify-center gap-2">
                    ...
                    <div className="px-6 py-4 bg-duck-bgLight border-t border-duck-ink/15 flex justify-end gap-3">
                        <button onClick={onClose} className="px-4 py-2.5 text-duck-ink/60 font-bold text-sm rounded-full border border-duck-ink hover:bg-duck-bg transition-colors">
                        ...
                        {!result && (
                            <button
                                ...
                                className="px-5 py-2.5 bg-duck-acid text-duck-ink font-bold text-sm rounded-full disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                            >
