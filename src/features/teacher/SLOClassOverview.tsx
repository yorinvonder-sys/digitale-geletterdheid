    ...
    return (
        <div className="space-y-6">
            ...
            {spotlightProgress.length > 0 && (
                <div className="bg-white rounded-xl border border-duck-ink/15 shadow-sm overflow-hidden">
                    <div className="px-4 py-3 border-b border-duck-ink/15 bg-duck-ink">
                        <p className="text-[10px] font-black text-duck-acid uppercase tracking-widest">Curriculum Spotlight</p>
                    ...
                    <div className="p-4 space-y-3">
                        {spotlightProgress.map(({ spotlight, totalStudents, startedStudents, completedStudents, inProgressStudents, startedPercentage, completedPercentage, classBreakdown, signals }) => (
                            <div key={spotlight.missionId} className="rounded-xl border border-duck-ink/15 bg-duck-bgLight p-4">
                                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {spotlight.kerndoelen.map(code => (
                                            <span key={code} className="inline-flex px-2 py-1 rounded-full bg-duck-acid text-duck-ink text-[11px] font-bold">
                                                {code}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="mt-3 grid gap-3 md:grid-cols-4">
                                    ...
                                    <div className="rounded-lg bg-white border border-duck-ink/20 px-3 py-2">
                                        <p className="text-[11px] font-bold text-duck-ink/60 uppercase tracking-wider">Gestart</p>
                                        <p className="text-xl font-black text-duck-ink mt-1">{startedStudents}</p>
                                        <p className="text-xs text-duck-ink/60 mt-1">{startedPercentage}% van leerjaarfilter</p>
                                    </div>
                                    <div className="rounded-lg bg-white border border-duck-ink/20 px-3 py-2">
                                        <p className="text-[11px] font-bold text-duck-ink/60 uppercase tracking-wider">Nog Bezig</p>
                                        <p className="text-xl font-black text-duck-ink/60 mt-1">{inProgressStudents}</p>
                                        <p className="text-xs text-duck-ink/60 mt-1">Wel gestart, nog niet afgerond</p>
                                    </div>
                                    <div className="rounded-lg bg-duck-acid border border-duck-acid px-3 py-2">
                                ...
                                <div className="mt-3 grid gap-2">
                                    <div>
                                        ...
                                        <div className="mt-1 h-2 rounded-full bg-duck-ink/10 overflow-hidden">
                                            <div className="h-full rounded-full bg-duck-ink/40 transition-all" style={{ width: `${startedPercentage}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex items-center justify-between text-[11px] font-bold text-duck-ink/60 uppercase tracking-wider">
                                        ...
                                        <div className="mt-1 h-2 rounded-full bg-duck-ink/10 overflow-hidden">
                                            <div className="h-full rounded-full bg-duck-acid transition-all" style={{ width: `${completedPercentage}%` }} />
                                        </div>
                                    </div>
                                </div>
                                {signals.length > 0 && (
                                            ...
                                            {signals.map(signal => {
                                                const toneStyles = signal.tone === 'success'
                                                    ? {
                                                        ...
                                                        icon: 'bg-duck-ink text-duck-acid',
                                                        title: 'text-duck-ink',
                                                        nudge: 'bg-white/80 border-duck-ink/20 text-duck-ink',
                                                    }
                                                    : signal.tone === 'attention'
                                                        ? {
                                                            wrapper: 'border-duck-ink/20 bg-duck-bgLight',
                                                            icon: 'bg-duck-ink/10 text-duck-ink',
                                                            title: 'text-duck-ink',
                                                            nudge: 'bg-white/80 border-duck-ink/15 text-duck-ink/60',
                                                        }
                                                        : {
                                                            wrapper: 'border-duck-error bg-duck-error/10',
                                                            icon: 'bg-duck-error text-white',
                                                            title: 'text-duck-error',
                                                            nudge: 'bg-white/80 border-duck-error/20 text-duck-error',
                                                        };
                                ...
                                <div className="mt-3 rounded-lg bg-white border border-duck-ink/15 px-3 py-3">
                                    ...
                                    <div className="mt-3 grid gap-3 lg:grid-cols-2">
                                        {classBreakdown.map(classProgress => (
                                            <div key={`${spotlight.missionId}-${classProgress.className}`} className="rounded-lg border border-duck-ink/15 bg-duck-bgLight px-3 py-3">
                                                <div className="flex items-start justify-between gap-3">
                                                    ...
                                                    <span className="inline-flex px-2 py-1 rounded-full bg-duck-acid text-duck-ink text-[11px] font-bold">
                                                        {classProgress.completedPercentage}%
                                                    </span>
                                                </div>
                                                <div className="mt-3 grid grid-cols-3 gap-2">
                                                    <div className="rounded-lg bg-white border border-duck-ink/15 px-2 py-2 text-center">
                                                        ...
                                                        <p className="text-lg font-black text-duck-ink/60 mt-1">{classProgress.startedStudents}</p>
                                                    </div>
                                                    <div className="rounded-lg bg-white border border-duck-ink/15 px-2 py-2 text-center">
                                                        <p className="text-[10px] font-bold text-duck-ink/60 uppercase tracking-wider">Bezig</p>
                                                        <p className="text-lg font-black text-duck-ink/40 mt-1">{classProgress.inProgressStudents}</p>
                                                    </div>
                                                    <div className="rounded-lg bg-duck-acid border border-duck-acid px-2 py-2 text-center">
                                                        <p className="text-[10px] font-bold text-duck-ink uppercase tracking-wider">Afgerond</p>
                    ...
                    return (
                        <div key={className} className="bg-white rounded-xl border border-duck-ink/15 shadow-sm overflow-hidden">
                            <button
                                ...
                                className="w-full p-4 flex items-center justify-between hover:bg-duck-bgLight transition-colors"
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-duck-ink rounded-lg flex items-center justify-center">
                                    ...
                                    <div className="hidden md:flex gap-2">
                                        {KERNDOEL_CODES.filter(code => classStats[code].hasData).map(code => (
                                            <div key={code} className="w-16" title={`${code} ${SLO_KERNDOELEN[code].label}`}>
                                                <div className="h-2 bg-duck-ink/10 rounded-full overflow-hidden">
                                                    <div
                                                        className="h-full transition-all bg-duck-acid"
                            ...
                            {isExpanded && (
                                <div className="border-t border-duck-ink/15">
                                    <table className="w-full text-sm">
                                        <thead className="bg-duck-bgLight">
                                            <tr>
                                                <th className="text-left px-4 py-2 text-xs font-bold text-duck-ink/60 uppercase">Leerling</th>
                                                {KERNDOEL_CODES.filter(code => classStats[code].hasData).map(code => (
                                                    <th key={code} className="text-center px-2 py-2 text-xs font-bold uppercase whitespace-nowrap text-duck-ink/60">
                                                        {code}
                                                    </th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-duck-ink/10">
                                            {classStudents.map(student => {
                                                const st = calculateStudentKerndoelStats(student, selectedYear);
                                                return (
                                                    <tr
                                                        key={student.uid}
                                                        className="hover:bg-duck-bgLight cursor-pointer transition-colors"
                                                    ...
                                                    >
                                                        <td className="px-4 py-3">
                                                            <div className="flex items-center gap-2">
                                                                <p className="font-medium text-duck-ink underline decoration-duck-ink/40 underline-offset-2">
                                                                    {student.displayName}
                                                                </p>
                                                                {student.stats?.vsoProfile && (
                                                                    <span className="text-[8px] bg-duck-acid text-duck-ink px-1 rounded border border-duck-acid font-bold uppercase">
                                                                        VSO
                                                                    </span>
                                                                )}
                                                            </div>
                                                        ...
                                                        {KERNDOEL_CODES.filter(code => classStats[code].hasData).map(code => {
                                                            ...
                                                            return (
                                                                <td key={code} className="text-center px-2 py-3">
                                                                    <span className={`inline-block px-2 py-1 rounded text-xs font-bold ${percentage >= 75 ? 'bg-duck-acid text-duck-ink' :
                                                                        percentage >= 50 ? 'bg-duck-ink/20 text-duck-ink' :
                                                                            percentage >= 25 ? 'bg-duck-ink/10 text-duck-ink' :
                                                                                'bg-duck-ink/5 text-duck-ink/50'
                                                                        }`}>
                                                                        {percentage}%
                                                                    </span>
                                                                </td>
