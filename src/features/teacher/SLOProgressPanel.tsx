        ...
        <div className="space-y-6">
            <div className="bg-lab-coral border border-lab-coral rounded-xl p-4 mb-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <h3 className="text-lab-coral font-bold flex items-center gap-2 mb-2">
                            <Info size={18} />
                    ...
                    <div className="bg-white px-3 py-2 rounded-lg border border-lab-coral shadow-sm shrink-0">
                        <div className="flex items-center gap-2 text-lab-coral mb-1">
                            <GraduationCap size={16} />
                            <span className="text-xs font-bold uppercase tracking-wider">Klas</span>
                        </div>
                        <div className="text-lg font-black text-lab-coral leading-none">
                            {studentClass}
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {KERNDOEL_CODES.map(code => {
                    ...
                    return (
                        <div key={code} className="bg-white rounded-xl border border-lab-line shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                            <div className="p-4">
                                <div className="flex items-start justify-between mb-3 gap-3">
                                    <div className="min-w-0">
                                        <h4 className="font-bold text-lab-ink truncate">
                                    ...
                                    <div className="text-right shrink-0">
                                        <span className={`text-2xl font-black ${stat.percentage >= 100 ? 'text-lab-muted' : 'text-lab-muted'}`}>
                                            {stat.percentage}%
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    {/* Progress Bar */}
                                    <div className="h-2 w-full bg-lab-cream rounded-full overflow-hidden">
                                        <div
                                            className={`h-full transition-all duration-500 ease-out ${stat.percentage >= 100 ? 'bg-lab-coral' : 'bg-lab-coral'}`}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {stat.completedMissions.map(mId => {
                                            ...
                                            return (
                                                <span
                                                    key={mId}
                                                    className="inline-flex items-center gap-1 px-2 py-1 bg-white border border-lab-line rounded text-xs text-lab-muted"
                                                    title={mId}
                                                >
                                                    <Check size={10} className="text-lab-muted" />
                                                    {label}
                                                </span>
                                            );
                                        })}
