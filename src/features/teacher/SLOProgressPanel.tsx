        ...
        <div className="space-y-6">
            <div className="bg-duck-error border border-duck-error rounded-xl p-4 mb-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <h3 className="text-duck-error font-bold flex items-center gap-2 mb-2">
                            <Info size={18} />
                            SLO Kerndoelen Digitale Geletterdheid
                        </h3>
                        <p className="text-duck-error text-sm">
                            Voortgang per officieel kerndoel (21A t/m 23C), gebaseerd op afgeronde missies.
                        </p>
                    </div>
                    <div className="bg-white px-3 py-2 rounded-lg border border-duck-error shadow-sm shrink-0">
                        <div className="flex items-center gap-2 text-duck-error mb-1">
                            <GraduationCap size={16} />
                            <span className="text-xs font-bold uppercase tracking-wider">Klas</span>
                        </div>
                        <div className="text-lg font-black text-duck-error leading-none">
                            {studentClass}
                        </div>
                    </div>
                </div>
                            ...
                            <div className="p-4">
                                <div className="flex items-start justify-between mb-3 gap-3">
                                    ...
                                    <div className="text-right shrink-0">
                                        <span className={`text-2xl font-black ${stat.percentage >= 100 ? 'text-duck-ink/60' : 'text-duck-ink/60'}`}>
                                            {stat.percentage}%
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    {/* Progress Bar */}
                                    <div className="h-2 w-full bg-duck-bg rounded-full overflow-hidden">
                                        <div
                                            className={`h-full transition-all duration-500 ease-out ${stat.percentage >= 100 ? 'bg-duck-error' : 'bg-duck-error'}`}
