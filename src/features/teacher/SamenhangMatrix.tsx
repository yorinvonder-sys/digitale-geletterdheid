...
function getIntensityClass(count: number): string {
    if (count === 0) return 'bg-duck-bg text-duck-ink/60';
    if (count <= 2) return 'bg-duck-ink text-white';
    if (count <= 4) return 'bg-duck-ink text-white';
    return 'bg-duck-error text-white';
}
...
export const SamenhangMatrix: React.FC<SamenhangMatrixProps> = ({ selectedYear, schoolId }) => {
    ...
    return (
        <motion.div
            ...
            className="bg-white rounded-[2rem] shadow-xl shadow-duck-ink/15/50 border border-duck-ink/15 overflow-hidden"
        >
            {/* Header */}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="px-3 py-2 bg-white border border-duck-ink/15 rounded-xl text-xs font-bold text-duck-ink/60 outline-none hover:bg-duck-bg transition-colors"
                        >
                            ...
                            <React.Fragment key={row.key}>
                                <tr>
                                    <td className="py-1.5 pr-3">
                                        ...
                                        <div className="text-[10px] text-duck-ink/60 truncate max-w-[160px]">{row.sublabel}</div>
                                    </td>
                                    {CATEGORIEEN.map(cat => {
                                            ...
                                            <td key={cat} className="py-1.5 px-2">
                                                <button
                                                    ...
                                                    className={`w-full rounded-xl py-3 px-2 text-center font-black text-lg transition-all ${getIntensityClass(cell.count)} ${
                                                        cell.count > 0
                                                            ? 'cursor-pointer hover:ring-2 hover:ring-duck-ink/40 hover:scale-105 active:scale-95'
                                                            : 'cursor-default'
                                                    } ${isExpanded ? 'ring-2 ring-duck-error' : ''}`}
                                                >
                                                    {cell.count}
                                                </button>
                                            </td>
                                        ...
                                        return (
                                            <tr key={`detail-${cellKey}`}>
                                                <td colSpan={4} className="pb-3">
                                                    ...
                                                    >
                                                        <div className="bg-duck-bg rounded-xl border border-duck-ink/15 p-4 mt-1">
                                                            <div className="flex items-center gap-2 mb-3">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="ml-auto text-[10px] font-bold text-duck-ink/60 hover:text-duck-ink/60 transition-colors"
                                                                >
                                                                    Sluiten
                                                                </button>
                                                            </div>
                                                            <div className="space-y-2">
                                                                {cell.details.map(detail => (
                                                                    <div
                                                                        ...
                                                                        className="bg-white rounded-lg border border-duck-ink/15 p-3"
                                                                    >
                                                                        ...
                                                                        <div className="mt-1.5 space-y-1">
                                                                            {detail.tags.map((tag, i) => (
                                                                                <div key={i} className="flex items-start gap-2 text-[11px]">
                                                                                    <span className="inline-block mt-0.5 w-1.5 h-1.5 rounded-full bg-duck-ink shrink-0"></span>
                                                                                    <span>
            ...
            <div className="px-6 pb-4">
                <div className="flex items-center gap-3 text-[9px] font-bold text-duck-ink/60 uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-1.5">
                        <div className="w-4 h-4 rounded bg-duck-bg border border-duck-ink/15"></div>
                        <span>0</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                        <div className="w-4 h-4 rounded bg-duck-ink"></div>
                        <span>1-2</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                        <div className="w-4 h-4 rounded bg-duck-ink"></div>
                    ...
                    <div className="flex items-center gap-1.5">
                        <div className="w-4 h-4 rounded bg-duck-error"></div>
                        <span>5+</span>
                    </div>
                </div>
            </div>
