...
export const StepOverrideModal: React.FC<StepOverrideModalProps> = ({
    ...
    const getStatusBadge = (status: ReturnType<typeof getStepStatus>) => {
        switch (status) {
            case 'ai_completed':
                return { label: 'AI voltooid', className: 'bg-duck-ink text-white', icon: <CheckCircle2 size={10} /> };
            case 'teacher_approved':
                return { label: 'Docent goedgekeurd', className: 'bg-duck-error text-white', icon: <Shield size={10} /> };
            case 'teacher_rejected':
                return { label: 'Docent afgewezen', className: 'bg-duck-error text-white', icon: <XCircle size={10} /> };
            case 'not_completed':
                return { label: 'Niet voltooid', className: 'bg-duck-bg text-duck-ink/60', icon: null };
        }
    };
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                >
                    {/* Header */}
                    <div className="bg-gradient-to-r from-duck-error via-duck-ink to-duck-error px-6 py-5 text-white relative overflow-hidden shrink-0">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-5">
                        {success ? (
                            ...
                            >
                                <div className="w-16 h-16 bg-duck-ink rounded-full flex items-center justify-center mx-auto mb-4">
                            ...
                            <>
                                ...
                                <div className="bg-duck-error rounded-xl p-3 flex items-start gap-3">
                                    <AlertTriangle size={16} className="text-duck-error shrink-0 mt-0.5" />
                                    <p className="text-xs text-duck-error">
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`w-full text-left p-3 rounded-xl border-2 transition-all ${
                                                            isSelected
                                                                ? 'border-duck-error bg-duck-error/50 ring-2 ring-duck-error/20'
                                                                : 'border-duck-ink/15 bg-duck-bg hover:border-duck-ink/15 hover:bg-duck-bg'
                                                        }`}
                                                    >
                                                        <div className="flex items-center gap-3">
                                ...
                                <AnimatePresence>
                                    {selectedStep !== null && (
                                        ...
                                        >
                                            <div className="bg-duck-bg rounded-xl p-4 space-y-4 border border-duck-ink/15">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <button
                                                            ...
                                                            className={`flex items-center justify-center gap-2 p-3 rounded-xl font-bold text-sm transition-all ${
                                                                overrideType === 'approve'
                                                                    ? 'bg-duck-error text-white shadow-lg shadow-duck-ink ring-2 ring-duck-error/30'
                                                        ...
                                                        <button
                                                            ...
                                                            className={`flex items-center justify-center gap-2 p-3 rounded-xl font-bold text-sm transition-all ${
                                                                overrideType === 'reject'
                                                                    ? 'bg-duck-error text-white shadow-lg shadow-duck-error ring-2 ring-duck-error/30'
                                                ...
                                                <div>
                                                    ...
                                                    <textarea
                                                        ...
                                                        className="w-full p-3 border border-duck-ink/15 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-duck-error/20 focus:border-duck-error"
                                                        rows={3}
                                                    />
                                                </div>
                                                ...
                                                {/* Error */}
                                                {error && (
                                                    <div className="flex items-center gap-2 p-3 bg-duck-error border border-duck-error rounded-xl text-duck-error text-xs font-medium">
                    ...
                    {!success && selectedStep !== null && (
                        <div className="px-6 py-4 bg-duck-bg border-t border-duck-ink/15 flex justify-end gap-3 shrink-0">
                            <button
                                onClick={onClose}
                            ...
                            <button
                                ...
                                className={`px-5 py-2.5 font-bold text-sm rounded-xl transition-all flex items-center gap-2 shadow-lg ${
                                    overrideType === 'approve'
                                        ? 'bg-gradient-to-r from-duck-error to-duck-error text-white hover:from-duck-ink hover:to-duck-ink shadow-duck-ink'
                                        : 'bg-gradient-to-r from-duck-error to-duck-error text-white hover:from-duck-error hover:to-duck-error shadow-duck-error'
