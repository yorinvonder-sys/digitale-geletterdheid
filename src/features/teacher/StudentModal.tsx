...
export const StudentModal: React.FC<StudentModalProps> = ({ student, onClose, onMessage, onBadge, onReset, onHighlight, onAwardXP, onLiveView, onDelete }) => {
    ...
    const getPriorityColor = (priority: 'low' | 'medium' | 'high') => {
        switch (priority) {
            ...
            case 'low': return 'bg-duck-ink text-white border-duck-ink';
        }
    };
            ...
            <div ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="student-modal-title" className="bg-white rounded-t-2xl md:rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-duck-ink/15 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-duck-error rounded-xl flex items-center justify-center text-duck-error">
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-duck-bg rounded-xl">
                ...
                <div className="px-4 border-b border-duck-ink/15 flex gap-4 shrink-0">
                    <button
                        ...
                        className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'overview' ? 'border-duck-error text-duck-error' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                    >
                        Overzicht
                    </button>
                    <button
                        onClick={() => setActiveTab('missions')}
                        className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'missions' ? 'border-duck-error text-duck-error' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                    >
                        Missies
                    </button>
                    <button
                        onClick={() => setActiveTab('slo')}
                        className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'slo' ? 'border-duck-error text-duck-error' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                    >
                        SLO Doelen
                    </button>
                    <button
                        onClick={() => setActiveTab('overrides')}
                        className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-1.5 ${activeTab === 'overrides' ? 'border-duck-error text-duck-error' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                    >
                        <Shield size={11} />
                        Overrides
                    ...
                    <button
                        ...
                        className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'groei' ? 'border-duck-ink text-duck-ink' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                    >
                        Groei
                    </button>
                    {student.stats?.nulmetingResult && (
                        <button
                            ...
                            className={`py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'paspoort' ? 'border-duck-error text-duck-error' : 'border-transparent text-duck-ink/60 hover:text-duck-ink/60'}`}
                        >
                            Paspoort
                        </button>
                    )}
                    ...
                    {activeTab === 'missions' ? (
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                <div className="bg-duck-bg rounded-xl p-3 text-center">
                                ...
                                <div className="bg-duck-error rounded-xl p-3 text-center">
                                    <div className="text-xl font-black text-duck-error">{student.stats?.level || 1}</div>
                                    <div className="text-[9px] text-duck-error font-bold uppercase">Level</div>
                                </div>
                                <div className="bg-duck-ink rounded-xl p-3 text-center">
                                    <div className="text-xl font-black text-duck-ink">{student.stats?.missionsCompleted?.length || 0}</div>
                                    <div className="text-[9px] text-duck-ink font-bold uppercase">Missies</div>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-duck-ink/15 pt-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-1 px-2 py-1 bg-duck-error text-white rounded-lg text-xs font-bold hover:bg-duck-error hover:text-white transition-colors"
                                    >
                                        <Plus size={12} />
                                        Nieuw
                                    </button>
                                ...
                                <AnimatePresence>
                                    {showNoteForm && (
                                        ...
                                        >
                                            <div className="bg-duck-bg rounded-xl p-3 space-y-3">
                                                <textarea
                                                    value={noteText}
                                                    onChange={(e) => setNoteText(e.target.value)}
                                                    placeholder="Schrijf een notitie..."
                                                    className="w-full p-3 border border-duck-ink/15 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-duck-error/20 focus:border-duck-error"
                                                    rows={3}
                                                />
                                                    ...
                                                    {(['low', 'medium', 'high'] as const).map(p => (
                                                        <button
                                                            ...
                                                            className={`px-2 py-1 rounded-lg text-xs font-bold transition-all ${notePriority === p
                                                                ...
                                                                : 'bg-duck-bg text-duck-ink/60 hover:bg-duck-bg'
                                                                }`}
                                                        >
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        ...
                                                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-duck-error text-white rounded-lg text-xs font-bold hover:bg-duck-error hover:text-white transition-colors disabled:opacity-50"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-3 py-2 bg-duck-bg text-duck-ink/60 rounded-lg text-xs font-bold hover:bg-duck-bg transition-colors"
                                                    >
                                                        Annuleren
                                                    </button>
                                                </div>
                            ...
                            <div className="border-t border-duck-ink/15 pt-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            onClick={onMessage}
                                            className="flex items-center justify-center gap-2 p-3 bg-duck-error text-white rounded-xl font-bold text-xs hover:bg-duck-error hover:text-white transition-colors"
                                        >
                                            <Send size={14} /> Bericht
                                        </button>
                                        <button
                                            onClick={() => setShowXPModal(true)}
                                            className="flex items-center justify-center gap-2 p-3 bg-duck-ink text-white rounded-xl font-bold text-xs hover:bg-duck-ink hover:text-white transition-colors"
                                        >
                                            <Zap size={14} /> + XP
                                        </button>
                                        <button
                                            onClick={onBadge}
                                            className="flex items-center justify-center gap-2 p-3 bg-duck-acid text-duck-ink rounded-xl font-bold text-xs hover:bg-duck-acid hover:text-duck-ink transition-colors"
                                        >
                                            <Award size={14} /> Badge
                                        </button>
                                        <button
                                ...
                                <div>
                                    ...
                                    <button
                                        onClick={onLiveView}
                                        className="w-full flex items-center justify-center gap-2 p-3 bg-duck-ink text-white rounded-xl font-bold text-xs hover:bg-duck-ink hover:text-white transition-colors"
                                    >
                                ...
                                {/* Danger Zone */}
                                <div className="bg-duck-error/50 rounded-2xl p-3 border border-duck-error">
                                    <h3 className="text-[10px] font-black text-duck-error uppercase tracking-widest mb-3">Gevaarlijke Acties</h3>
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            onClick={onReset}
                                            className="flex items-center justify-center gap-2 p-2.5 bg-white text-duck-error border border-duck-error rounded-xl font-bold text-[10px] hover:bg-duck-error hover:text-white transition-colors"
                                        >
...
const MissionScoresView: React.FC<{
    ...
    const statusColor = (s: 'completed' | 'in_progress' | 'not_started') => {
        switch (s) {
            case 'completed': return 'bg-duck-ink text-white';
            case 'in_progress': return 'bg-duck-acid text-duck-ink';
            case 'not_started': return 'bg-duck-bg text-duck-ink/60';
        }
    };
    ...
    return (
        <div className="space-y-3">
            ...
            {/* Progress bar */}
            <div className="w-full bg-duck-bg rounded-full h-2">
                <div
                    className="bg-duck-error h-2 rounded-full transition-all"
            ...
            <div className="space-y-2">
                {allMissions.map(mission => {
                    ...
                    return (
                        <div key={mission.id} className="flex items-center gap-3 p-3 bg-duck-bg rounded-xl">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    {lastActive && (
                                        <span className="flex items-center gap-1 text-[9px] text-duck-ink/60">
                                            <Clock size={8} />
                            ...
                            {score !== null && score !== undefined && (
                                <div className="text-right mr-1">
                                    <div className={`text-lg font-black ${score >= 70 ? 'text-duck-ink' : score >= 50 ? 'text-duck-ink' : 'text-duck-ink/60'}`}>
                                        {score}%
                                    </div>
                                    <div className="text-[8px] font-bold text-duck-ink/60 uppercase">Score</div>
                                </div>
                            )}
                            {hasSteps && (
                                <button
                                    onClick={() => onOverride(mission.id, mission.name)}
                                    className="p-2 text-duck-error hover:text-duck-error hover:bg-duck-error hover:text-white rounded-lg transition-colors shrink-0"
...
const OverrideHistoryView: React.FC<{
    ...
    if (overrides.length === 0) {
        return (
            <div className="text-center py-8">
                <div className="w-12 h-12 bg-duck-bg rounded-full flex items-center justify-center mx-auto mb-3">
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-duck-error rounded-xl p-3 flex items-start gap-3">
                <AlertCircle size={14} className="text-duck-error shrink-0 mt-0.5" />
                <p className="text-[10px] text-duck-error">
                        ...
                        {missionOverrides.map(o => {
                            ...
                            return (
                                <div key={o.id} className={`p-3 rounded-xl border ${
                                    ...
                                    <p className="text-[9px] text-duck-ink/60 mt-1 flex items-center gap-1">
                                        <Clock size={8} />
