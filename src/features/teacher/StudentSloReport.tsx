            ...
            <div className="relative bg-white w-full max-w-4xl rounded-none md:rounded-2xl shadow-2xl my-0 md:my-4 print:shadow-none print:max-w-none print:rounded-none print:my-0">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between gap-3 p-4 md:p-5 bg-white border-b border-duck-ink/15 rounded-t-2xl print:hidden">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="w-10 h-10 rounded-xl bg-duck-error text-white flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-duck-ink/15 text-duck-ink/60 hover:border-duck-error hover:text-duck-error rounded-lg text-xs font-semibold transition-colors"
                        ...
                        <button
                            type="button"
                            onClick={handlePrint}
                            className="inline-flex items-center gap-1.5 px-3 py-2 bg-duck-error text-white hover:bg-duck-error hover:text-white rounded-lg text-xs font-semibold transition-colors"
                        ...
                        <button
                            type="button"
                            onClick={onClose}
                            className="p-2 text-duck-ink/60 hover:text-duck-ink/60 transition-colors rounded-lg hover:bg-duck-bg"
                ...
                <article className="p-6 md:p-10 print:p-8 text-duck-ink">
                    ...
                    <header className="border-b-2 border-duck-ink/15 pb-4 mb-8 flex flex-wrap items-end justify-between gap-3">
                        <div>
                            <p className="text-[10px] font-black tracking-widest uppercase text-duck-error">DGSkills — SLO-leerlingrapport</p>
                    ...
                    {applicableCodes.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-duck-ink/15 bg-duck-bg p-8 text-center">
...
const StatBlock: React.FC<{ label: string; value: React.ReactNode; tone?: StatTone }> = ({ label, value, tone = 'neutral' }) => {
    const toneClass =
        tone === 'success' ? 'text-duck-ink'
            : tone === 'warn' ? 'text-duck-ink'
                : 'text-duck-ink';
    return (
        <div className="rounded-xl bg-duck-bg border border-duck-ink/15 px-4 py-3 print:border-duck-ink/15">
            <p className="text-[10px] font-bold uppercase tracking-wider text-duck-ink/60 mb-1">{label}</p>
...
const KerndoelRow: React.FC<KerndoelRowProps> = ({ code, completed, total, percentage, completedMissions, totalMissions }) => {
    ...
    return (
        <li className="border border-duck-ink/15 rounded-xl p-5 print:border-duck-ink/15 print:break-inside-avoid">
            <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {kd.isVso && (
                            <span className="text-[10px] font-bold text-white bg-duck-ink border border-duck-ink px-1.5 py-0.5 rounded uppercase tracking-wider">VSO</span>
                        )}
                    </div>
            ...
            {/* Voortgangsbalk */}
            <div className="h-2 bg-duck-bg rounded-full overflow-hidden mb-4 print:border print:border-duck-ink/15" aria-hidden="true">
