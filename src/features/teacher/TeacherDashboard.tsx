        ...
        >
            ...
            <div className="min-h-screen overflow-x-hidden bg-duck-bg text-duck-ink">
                    ...
                    <AnimatePresence>
                        {toasts.map(t => (
                            <motion.div key={t.id} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="pointer-events-auto min-w-[300px] p-4 rounded-xl shadow-xl bg-white border border-duck-ink/15 flex items-start gap-3">
                                <div className={`p-2 rounded-lg ${t.type === 'error' ? 'bg-duck-error/10 text-duck-error' : 'bg-duck-bg text-duck-ink'}`}><Stars size={20} /></div>
                ...
                <div className="min-h-screen lg:grid lg:grid-cols-[216px_minmax(0,1fr)]">
                    <aside className="hidden border-r border-duck-ink/15 bg-duck-bg lg:flex lg:flex-col">
                            ...
                            {sideNavItems.map((item, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {item.badge ? (
                                            <span className="rounded-full bg-duck-ink px-2 py-0.5 text-[10px] font-black text-white">{item.badge}</span>
                                        ) : null}
                                    </button>
                                );
                            })}
                    ...
                    <div className="min-w-0">
                        <header className="sticky top-0 z-40 border-b border-duck-ink/15 bg-duck-bg/95 backdrop-blur">
                                ...
                                <div className="ml-auto flex min-w-0 shrink-0 items-center gap-2 sm:gap-3">
                                    <div ref={classDropdownRef} className="relative">
                                        <button
                                            ...
                                            className="flex h-11 min-w-[142px] max-w-[172px] items-center justify-between gap-2 rounded-xl border border-duck-ink/15 bg-duck-bg px-3 text-left text-sm font-black text-duck-ink outline-none transition hover:border-duck-ink sm:h-12 sm:min-w-[172px] sm:px-4"
                                        ...
                                        <AnimatePresence>
                                            {classDropdownOpen && (
                                                <motion.div
                                                    ...
                                                    className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-xl border border-duck-ink/15 bg-duck-bg p-1 shadow-lg"
                                                    role="listbox"
                                                >
                                    ...
                                    <button className="relative hidden h-11 w-11 items-center justify-center rounded-full text-duck-ink hover:bg-duck-bg sm:flex" aria-label="Meldingen">
                                        <Bell size={22} />
                                        {attentionCount > 0 && (
                                            <span className="absolute right-1 top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-duck-ink px-1 text-[10px] font-black text-white">
                                                {attentionCount}
                                            </span>
                                        )}
                                    </button>
                                    <button
                                        ...
                                        className="hidden h-11 items-center gap-2 rounded-xl bg-duck-ink px-4 text-sm font-black text-white transition hover:bg-duck-ink/80 md:flex"
                                    >
                        ...
                        {activeTab === 'students' && (
                            <PageTransition key="students" className="space-y-4">
                                <div className="bg-duck-bg rounded-xl border border-duck-ink/15 p-3 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => setShowRosterImport(true)} className="px-4 py-2 bg-duck-bg border border-duck-ink/15 text-duck-ink rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-duck-bg"><Upload size={14} /> Importeren</button>
                                        <button data-tutorial="students-message-btn" onClick={() => setShowMessageModal(true)} className="px-4 py-2 bg-duck-ink text-white rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-duck-ink/80"><Send size={14} /> Bericht</button>
                                    </div>
                                </div>
                                <StudentList
