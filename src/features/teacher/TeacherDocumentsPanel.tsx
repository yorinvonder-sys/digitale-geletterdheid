...
const CATEGORY_COLORS: Record<SloDocument['category'], { bg: string; text: string; border: string; iconBg: string }> = {
    slo: { bg: 'bg-duck-acid', text: 'text-duck-ink', border: 'border-duck-ink', iconBg: 'bg-duck-acid' },
    compliance: { bg: 'bg-duck-ink/5', text: 'text-duck-ink/60', border: 'border-duck-ink/15', iconBg: 'bg-duck-ink/5' },
    handleiding: { bg: 'bg-duck-ink/5', text: 'text-duck-ink/60', border: 'border-duck-ink/15', iconBg: 'bg-duck-ink/5' },
};
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-lg font-bold text-duck-ink flex items-center gap-2">
                            <BookOpen size={20} className="text-duck-ink/60" />
                            Documenten
                        </h2>
                    ...
                    <div className="relative w-full sm:w-72">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-duck-ink/40" size={16} />
                        <input
                            type="text"
                            placeholder="Zoek documenten..."
                            className="w-full pl-9 pr-4 py-2.5 bg-duck-bg border border-duck-ink/15 rounded-xl text-sm focus:ring-2 focus:ring-duck-ink/10 outline-none transition-all"
                            value={searchQuery}
                        ...
                        {categories.map(cat => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeCategory === cat
                                    ...
                                    : 'text-duck-ink/60 hover:text-duck-ink hover:bg-duck-bgLight'
                                }`}
                            >
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-duck-ink/15 mt-auto">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-duck-ink hover:bg-duck-acid rounded-xl transition-colors"
                                            >
                                            ...
                                            <a
                                                href={doc.path}
                                                download
                                                className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-duck-ink/60 hover:bg-duck-bgLight rounded-xl transition-colors"
                                            >
                                                <Download size={14} />
                                                Download
                                            </a>
            ...
            {filteredDocs.length === 0 && (
                <div className="bg-white rounded-2xl border border-duck-ink/15 p-12 text-center">
                    <BookOpen size={40} className="text-duck-ink/40 mx-auto mb-3" />
            ...
            <div className="bg-duck-bgLight rounded-2xl p-5 flex items-start gap-4 border border-duck-ink/15">
                <div className="bg-duck-acid p-2 rounded-xl shrink-0">
                ...
                <div>
                    ...
                    <button
                        ...
                        className="mt-3 text-xs font-bold text-duck-ink hover:underline flex items-center gap-1"
                    >
