    ...
    return (
        <div className="fixed inset-0 z-[100] bg-duck-ink/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden relative animate-in zoom-in duration-300">
                ...
                <div className="p-10 pt-14 text-center">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <button
                            ...
                            className={`p-4 rounded-2xl text-duck-ink/60 hover:bg-duck-bgLight transition-all ${currentSlide === 0 ? 'opacity-0' : 'opacity-100'}`}
                        >
                            <ChevronLeft size={24} />
                        </button>
                        ...
                        <button
                            onClick={next}
                            className={`flex-1 py-4 ${slide.color} text-duck-ink rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg hover:scale-105 transition-all active:scale-95`}
                        >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 p-2 text-duck-ink/60 hover:text-duck-ink transition-colors"
                >
                    <X size={20} />
                </button>
            </div>
...
export const TeacherInfoButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
    <button
        onClick={onClick}
        className="fixed bottom-6 right-6 w-10 h-10 bg-white border border-duck-ink/15 text-duck-ink/60 hover:text-duck-ink hover:border-duck-ink rounded-full flex items-center justify-center shadow-lg transition-all hover:scale-110 z-40 group"
        title="Info & Uitleg"
    >
        <Info size={18} />
