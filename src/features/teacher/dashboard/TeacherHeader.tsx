...
export const TeacherHeader: React.FC<TeacherHeaderProps> = ({
    ...
    return (
        <header className="max-w-7xl mx-auto mb-6">
            <div className="bg-white rounded-2xl border border-lab-line shadow-sm p-4 md:p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    {/* Left: Title */}
                    <div>
                        <h1 className="text-2xl font-black text-lab-ink tracking-tight">
                            Docenten Dashboard
                        </h1>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <div data-tutorial="focus-toggle" className="flex items-center gap-2 bg-lab-cream px-3 py-2 rounded-xl border border-lab-line group relative">
                            ...
                            <button
                                ...
                                className={`transition-all duration-300 ${focusMode ? 'text-lab-coral' : 'text-lab-muted'}`}
                            >
                                {focusMode ? <ToggleRight size={24} /> : <ToggleLeft size={24} />}
                            </button>
                            {focusMode && focusModeRemaining > 0 && (
                                <span className="text-[10px] font-bold text-white bg-lab-coral px-2 py-0.5 rounded-full" aria-live="polite">
                                    {Math.floor(focusModeRemaining / 60)}:{String(focusModeRemaining % 60).padStart(2, '0')}
                                </span>
                            )}
                            {/* Tooltip */}
                            <div className="absolute top-full right-0 mt-2 w-48 bg-lab-ink text-white text-[10px] p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-xl">
                        ...
                        <div className="group relative">
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-lab-coral to-lab-coral text-white rounded-xl hover:from-lab-teal hover:to-lab-coral transition-colors text-sm font-bold"
                            >
                                <MonitorPlay size={16} />
                        ...
                        {onLogout && (
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-2 bg-lab-cream text-lab-muted rounded-xl text-sm font-bold hover:bg-lab-coral hover:text-white transition-colors"
                            >
                                <LogOut size={16} />
                            </button>
                        )}
