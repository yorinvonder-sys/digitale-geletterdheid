                        ...
                        <div className="space-y-4">
                                    ...
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition-all ${messageTarget === opt.id
                                                ...
                                                : 'border-duck-ink/15 text-duck-ink/60 hover:border-duck-ink/15'
                                                }`}
                                        >
                                            <opt.icon size={18} />
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full p-3 border border-duck-ink/15 rounded-xl text-sm resize-none h-28 focus:ring-2 focus:ring-duck-error/20 focus:border-duck-error outline-none"
                                ></textarea>
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    ...
                                    className="flex-1 py-3 text-duck-ink/60 font-bold rounded-xl hover:bg-duck-bg transition-colors"
                                >
                                    Annuleer
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-3 bg-duck-error text-white font-bold rounded-xl hover:bg-duck-error hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                                >
                                    <Send size={16} />
                                    Verstuur
                                </button>
                ...
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowBadgeModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="Badge toekennen" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-4 gap-2 mb-4">
                            {AVAILABLE_BADGES.map(badge => (
                                <button
                                    ...
                                    className={`p-3 rounded-xl text-2xl transition-all ${selectedBadge === badge.id ? 'bg-duck-acid ring-2 ring-duck-acid' : 'bg-duck-bg hover:bg-duck-bg'}`}
                                    title={badge.name}
                                >
                                    {badge.emoji}
                                </button>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowBadgeModal(false)} className="flex-1 py-2 text-duck-ink/60 font-bold rounded-xl hover:bg-duck-bg">Annuleer</button>
                            <button onClick={handleAwardBadge} disabled={!selectedBadge} className="flex-1 py-2 bg-duck-error text-white font-bold rounded-xl hover:bg-duck-acid hover:text-duck-ink disabled:opacity-50">Toekennen</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showResetConfirm && selectedStudent && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowResetConfirm(false)}>
                    <div role="alertdialog" aria-modal="true" aria-label="Bevestig reset" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-2 text-duck-ink/60 font-bold rounded-xl hover:bg-duck-bg">Annuleer</button>
                ...
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHighlightModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="Werk uitlichten" className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-8" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowHighlightModal(false)} className="flex-1 py-4 text-duck-ink/60 font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-duck-bg transition-all">Annuleer</button>
                            <button
                                onClick={handleHighlightWork}
                                disabled={!highlightTitle || !highlightContent}
                                className="flex-1 py-4 bg-duck-error text-white font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-duck-acid hover:text-duck-ink shadow-lg shadow-duck-acid disabled:opacity-50 transition-all"
                            >
                                In de Gallerij
                            </button>
                        </div>
                ...
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowEventModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="XP Boost Event" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setShowEventModal(false)} className="flex-1 py-2 text-duck-ink/60 font-bold rounded-xl hover:bg-duck-bg">Annuleer</button>
                                <button onClick={handleCreateEvent} className="flex-1 py-2 bg-gradient-to-r from-duck-error to-duck-error text-white font-bold rounded-xl hover:shadow-lg">Start Event</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setStudentToDelete(null)}>
                    <div role="alertdialog" aria-modal="true" aria-label="Bevestig verwijderen" className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200 motion-reduce:animate-none" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-center w-12 h-12 mx-auto bg-duck-error rounded-full mb-4">
                            <AlertTriangle className="text-duck-error" size={24} />
                        </div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-2.5 bg-duck-bg text-duck-ink/60 font-bold rounded-xl hover:bg-duck-bg transition-colors"
                                disabled={isDeleting}
                            >
                                Annuleren
                            </button>
