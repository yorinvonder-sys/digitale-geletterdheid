            ...
            {showMessageModal && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowMessageModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="Bericht versturen" className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-black text-lab-ink mb-4 flex items-center gap-2">
                            <Send className="text-lab-muted" size={20} />
                            Bericht Versturen
                        </h3>
                        <div className="space-y-4">
                            {/* Target Type Selector */}
                            <div>
                                <label className="text-xs font-bold text-lab-muted uppercase mb-2 block">Verstuur naar</label>
                                    ...
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition-all ${messageTarget === opt.id
                                                ...
                                                : 'border-lab-line text-lab-muted hover:border-lab-line'
                                                }`}
                                        >
                                            <opt.icon size={18} />
                            ...
                            {(messageTarget === 'class' || messageTarget === 'student') && (
                                <div>
                                    <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">
                                    ...
                                    <select
                                        ...
                                        className="w-full p-3 border border-lab-line rounded-xl text-sm font-medium bg-white"
                                    >
                            ...
                            {messageTarget === 'student' && messageTargetClassId && (
                                <div>
                                    <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">Selecteer Leerling</label>
                                    <select
                            ...
                            {/* Message Text */}
                            <div>
                                <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">Bericht</label>
                                <textarea
                                    value={messageText}
                                    onChange={(e) => setMessageText(e.target.value)}
                                    placeholder="Typ je bericht..."
                                    className="w-full p-3 border border-lab-line rounded-xl text-sm resize-none h-28 focus:ring-2 focus:ring-lab-coral/20 focus:border-lab-coral outline-none"
                                ></textarea>
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    ...
                                    className="flex-1 py-3 text-lab-muted font-bold rounded-xl hover:bg-lab-cream transition-colors"
                                >
                                    Annuleer
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-3 bg-lab-coral text-white font-bold rounded-xl hover:bg-lab-coral hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                                >
                                    <Send size={16} />
                                    Verstuur
                                </button>
            ...
            {showBadgeModal && selectedStudent && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowBadgeModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="Badge toekennen" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-black text-lab-ink mb-4">Badge Toekennen</h3>
                        <div className="grid grid-cols-4 gap-2 mb-4">
                            {AVAILABLE_BADGES.map(badge => (
                                <button
                                    key={badge.id}
                                    onClick={() => setSelectedBadge(badge.id)}
                                    className={`p-3 rounded-xl text-2xl transition-all ${selectedBadge === badge.id ? 'bg-lab-gold ring-2 ring-lab-gold' : 'bg-lab-cream hover:bg-lab-cream'}`}
                                    title={badge.name}
                                >
                                    {badge.emoji}
                                </button>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowBadgeModal(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-cream">Annuleer</button>
                            <button onClick={handleAwardBadge} disabled={!selectedBadge} className="flex-1 py-2 bg-lab-coral text-white font-bold rounded-xl hover:bg-lab-gold hover:text-lab-ink disabled:opacity-50">Toekennen</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowResetConfirm(false)}>
                    <div role="alertdialog" aria-modal="true" aria-label="Bevestig reset" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        <div className="text-center mb-4">
                            ...
                            <h3 className="text-lg font-black text-lab-ink">Weet je het zeker?</h3>
                            <p className="text-sm text-lab-muted mt-2">Dit reset alle XP, level en voltooide missies van {selectedStudent.displayName}.</p>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-cream">Annuleer</button>
                            <button onClick={handleResetStudent} className="flex-1 py-2 bg-lab-coral text-white font-bold rounded-xl hover:bg-lab-coral hover:text-white">Reset</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showHighlightModal && selectedStudent && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHighlightModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="Werk uitlichten" className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-8" onClick={e => e.stopPropagation()}>
                        <div className="w-16 h-16 bg-lab-gold text-lab-ink rounded-2xl flex items-center justify-center mb-6">
                            <Trophy size={32} />
                        </div>
                        <h3 className="text-2xl font-black text-lab-ink mb-2">Kroon op het Werk</h3>
                        <p className="text-lab-muted font-medium mb-6">Highlight een bijzondere prestatie van {selectedStudent.displayName} voor de hele klas.</p>

                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-line px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Titel van de prestatie</label>
                                <input
                                    type="text"
                                    value={highlightTitle}
                                    onChange={(e) => setHighlightTitle(e.target.value)}
                                    placeholder="Bijv. Meesterlijke Prompt..."
                                    className="w-full p-4 border-2 border-lab-line rounded-2xl text-sm font-bold focus:border-lab-gold outline-none transition-all"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-line px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Inhoud / Prompt</label>
                                <textarea
                                    value={highlightContent}
                                    onChange={(e) => setHighlightContent(e.target.value)}
                                    placeholder="Wat was er zo goed?"
                                    className="w-full p-4 border-2 border-lab-line rounded-2xl text-sm font-bold focus:border-lab-gold outline-none transition-all resize-none h-32"
                                ></textarea>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-lab-muted border border-lab-line px-2 py-1 rounded-lg uppercase tracking-widest mb-2 inline-block">Jouw compliment (Optioneel)</label>
                                <input
                                    type="text"
                                    value={highlightNote}
                        ...
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowHighlightModal(false)} className="flex-1 py-4 text-lab-muted font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-lab-cream transition-all">Annuleer</button>
                            <button
                                onClick={handleHighlightWork}
                                disabled={!highlightTitle || !highlightContent}
                                className="flex-1 py-4 bg-lab-coral text-white font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-lab-gold hover:text-lab-ink shadow-lg shadow-lab-gold disabled:opacity-50 transition-all"
                            >
                                In de Gallerij
                            </button>
                        </div>
            ...
            {showEventModal && (
                <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowEventModal(false)}>
                    <div role="dialog" aria-modal="true" aria-label="XP Boost Event" className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-black text-lab-ink mb-4 flex items-center gap-2">
                            <Sparkles className="text-lab-muted" /> XP Boost Event
                        </h3>
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">Naam</label>
                                <input
                                    type="text"
                                    value={eventName}
                                    onChange={(e) => setEventName(e.target.value)}
                                    placeholder="Bijv. Vrijdag Bonus"
                                    className="w-full p-2 border border-lab-line rounded-xl text-sm"
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">Duur</label>
                                    <select
                                        value={eventDuration}
                                ...
                                <div>
                                    <label className="text-xs font-bold text-lab-muted uppercase mb-1 block">Multiplier</label>
                                    <select
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setShowEventModal(false)} className="flex-1 py-2 text-lab-muted font-bold rounded-xl hover:bg-lab-cream">Annuleer</button>
                                <button onClick={handleCreateEvent} className="flex-1 py-2 bg-gradient-to-r from-lab-coral to-lab-coral text-white font-bold rounded-xl hover:shadow-lg">Start Event</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {studentToDelete && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setStudentToDelete(null)}>
                    <div role="alertdialog" aria-modal="true" aria-label="Bevestig verwijderen" className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200 motion-reduce:animate-none" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-center w-12 h-12 mx-auto bg-lab-coral rounded-full mb-4">
                            <AlertTriangle className="text-lab-coral" size={24} />
                        </div>
                        <h3 className="text-lg font-black text-center text-lab-ink mb-2">
                            Leerling Verwijderen?
                        </h3>
                        <p className="text-center text-lab-muted text-sm mb-6">
                            Weet je zeker dat je <span className="font-bold text-lab-muted">{studentToDelete.displayName || 'deze leerling'}</span> wilt verwijderen? Dit kan niet ongedaan worden gemaakt.
                        </p>
                        <div className="flex gap-3">
                            <button
                                onClick={() => setStudentToDelete(null)}
                                className="flex-1 py-2.5 bg-lab-cream text-lab-muted font-bold rounded-xl hover:bg-lab-creamDeep transition-colors"
                                disabled={isDeleting}
                            >
                                Annuleren
                            </button>
                            <button
                                ...
                                className="flex-1 py-2.5 bg-lab-coral text-white font-bold rounded-xl hover:bg-lab-coral hover:text-white transition-colors flex items-center justify-center gap-2"
                                disabled={isDeleting}
                            >
