...
export const createLobby = async (lobby: Omit<BombermanLobby, 'id' | 'created_at' | 'updated_at'>): Promise<string> => {
    const { data, error } = await supabase
        ...
        .insert({
            ...lobby,
            players: lobby.players as unknown as Json,
            settings: lobby.settings as unknown as Json,
        })
        .select('id')
        .single();
...
export const getLobby = async (lobbyId: string): Promise<BombermanLobby | null> => {
    ...
    if (error) throw error;
    return data as unknown as BombermanLobby | null;
};
...
export const updateLobby = async (lobbyId: string, updates: Partial<BombermanLobby>): Promise<void> => {
    const { error } = await supabase
        ...
        .update({
            ...updates,
            players: updates.players as unknown as Json | undefined,
            settings: updates.settings as unknown as Json | undefined,
...
export const joinLobby = async (lobbyId: string, player: BombermanPlayer): Promise<boolean> => {
    try {
        ...
        const players = (lobby.players as unknown as BombermanPlayer[]) || [];
        ...
        const { error } = await supabase
            .from('bomberman_lobbies')
            .update({
                players: [...players, player] as unknown as Json,
                updated_at: new Date().toISOString(),
            })
            .eq('id', lobbyId);
...
export const leaveLobby = async (lobbyId: string, uid: string): Promise<void> => {
    try {
        ...
        const { error } = await supabase
            .from('bomberman_lobbies')
            .update({
                players: updated as unknown as Json,
                updated_at: new Date().toISOString(),
            })
            .eq('id', lobbyId);
...
export const getAvailableLobbies = async (schoolId?: string): Promise<BombermanLobby[]> => {
    ...
    if (error) throw error;
    return (data || []) as unknown as BombermanLobby[];
};
...
export const subscribeToActiveLobbies = (
    ...
    const fetchLobbies = async () => {
        ...
        const lobbies = (data || []).map(d => ({
            ...(d as unknown as BombermanLobby),
            playerCount: ((d.players as unknown as BombermanPlayer[]) || []).length,
        }));
        onUpdate(lobbies);
    };
