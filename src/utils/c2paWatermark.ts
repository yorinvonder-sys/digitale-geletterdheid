...
import type { C2paConfig } from 'c2pa';
import c2paWasmSrc from 'c2pa/dist/assets/wasm/toolkit_bg.wasm?url';
import c2paWorkerSrc from 'c2pa/dist/c2pa.worker.min.js?url';
...
const C2PA_CONFIG: C2paConfig = {
    wasmSrc: c2paWasmSrc,
    workerSrc: c2paWorkerSrc,
...
export async function verifyC2paCredentials(
    ...
    try {
        ...
        const c2pa = await createC2pa(C2PA_CONFIG);
...
async function embedBinaryC2pa(
    ...
    try {
        ...
        const c2pa = await createC2pa(C2PA_CONFIG);
        const signer = c2pa as unknown as {
            sign(input: unknown): Promise<{ signedAsset: BlobPart }>;
        };

        const { signedAsset } = await signer.sign({
            asset: {
