...
/**
 * Mask high-confidence personal data before forwarding free text to an external
 * AI provider. Deliberately conservative: only patterns with a low false-positive
 * rate are masked (e-mail, phone/BSN-like number runs, Dutch postcode). Names are
 * NOT redacted — reliable name detection is not feasible without harming tutoring.
 *
 * This is data minimisation, not prompt-injection defence (see sanitizePrompt).
 */
