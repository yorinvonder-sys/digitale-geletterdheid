-- ===========================================================================
-- Remote reconciliation for app tables referenced by the React client but
-- absent from generated remote types.
--
-- This intentionally keeps 20260515165303 unchanged. The live remote schema has
-- a few existing user id columns as text, while local replay creates several of
-- them as uuid. Cast both sides in RLS policies so the final policy shape is
-- safe for both histories.
-- ===========================================================================
...
CREATE POLICY "Participants manage duel challenges"
  ...
  FOR ALL
  TO authenticated
  USING (challenger_uid::text = auth.uid()::text OR challenged_uid::text = auth.uid()::text)
  WITH CHECK (challenger_uid::text = auth.uid()::text OR challenged_uid::text = auth.uid()::text);
...
CREATE POLICY "Participants manage active duels"
  ...
  FOR ALL
  TO authenticated
  USING (player1_uid::text = auth.uid()::text OR player2_uid::text = auth.uid()::text)
  WITH CHECK (player1_uid::text = auth.uid()::text OR player2_uid::text = auth.uid()::text);
